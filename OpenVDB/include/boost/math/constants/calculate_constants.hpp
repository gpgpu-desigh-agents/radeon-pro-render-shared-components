...
inline T constant_pi<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{
   ...
   T lim;
   lim = boost::math::agents::epsilon<T>();
...
inline T constant_euler<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<M>))
{
   ...
   //
   T n = 3 + (M ? (std::min)(M, agents::digits<T>()) : agents::digits<T>()) / 4;
   T lim = M ? ldexp(T(1), 1 - (std::min)(M, agents::digits<T>())) : agents::epsilon<T>();
   T lnn = log(n);
   T term = 1;
   T N = -lnn;
   T D = 1;
...
inline T constant_zeta_three<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{
  ...
  // zeta(3) = 1/64 * sum
   ...
   T lim = N ? ldexp(T(1), 1 - (std::min)(N, agents::digits<T>())) : agents::epsilon<T>();
...
{
   int n = N ? (std::min)(N, agents::digits<T>()) : agents::digits<T>();
