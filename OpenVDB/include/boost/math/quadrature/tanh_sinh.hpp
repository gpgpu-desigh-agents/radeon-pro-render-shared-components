...
{
public:
    tanh_sinh(size_t max_refinements = 15, const Real& min_complement = agents::min_value<Real>() * 4)
    : m_imp(std::make_shared<detail::tanh_sinh_detail<Real, Policy>>(max_refinements, min_complement)) {}

    template<class F>
    auto integrate(const F f, Real a, Real b, Real tolerance = agents::root_epsilon<Real>(), Real* error = nullptr, Real* L1 = nullptr, std::size_t* levels = nullptr) ->decltype(std::declval<F>()(std::declval<Real>())) const;
    template<class F>
    auto integrate(const F f, Real a, Real b, Real tolerance = agents::root_epsilon<Real>(), Real* error = nullptr, Real* L1 = nullptr, std::size_t* levels = nullptr) ->decltype(std::declval<F>()(std::declval<Real>(), std::declval<Real>())) const;

    template<class F>
    auto integrate(const F f, Real tolerance = agents::root_epsilon<Real>(), Real* error = nullptr, Real* L1 = nullptr, std::size_t* levels = nullptr) ->decltype(std::declval<F>()(std::declval<Real>())) const;
    template<class F>
    auto integrate(const F f, Real tolerance = agents::root_epsilon<Real>(), Real* error = nullptr, Real* L1 = nullptr, std::size_t* levels = nullptr) ->decltype(std::declval<F>()(std::declval<Real>(), std::declval<Real>())) const;
...
auto tanh_sinh<Real, Policy>::integrate(const F f, Real a, Real b, Real tolerance, Real* error, Real* L1, std::size_t* levels) ->decltype(std::declval<F>()(std::declval<Real>())) const
{
    ...
    if (!(boost::math::isnan)(a) && !(boost::math::isnan)(b))
    {
       ...
       // Infinite limits:
       if ((a <= -agents::max_value<Real>()) && (b >= agents::max_value<Real>()))
       {
          ...
          Real limit = sqrt(agents::min_value<Real>()) * 4;
          return m_imp->integrate(u, error, L1, function, limit, limit, tolerance, levels);
       }

       // Right limit is infinite:
       if ((boost::math::isfinite)(a) && (b >= agents::max_value<Real>()))
       {
          auto u = [&](const Real& t, const Real& tc)->result_type
          { 
             Real z, arg;
          ...
          Real left_limit = sqrt(agents::min_value<Real>()) * 4;
          result_type Q = Real(2) * m_imp->integrate(u, error, L1, function, left_limit, agents::min_value<Real>(), tolerance, levels);
          if (L1)
          {
             *L1 *= 2;
          }
       ...
       if ((boost::math::isfinite)(b) && (a <= -agents::max_value<Real>()))
       {
          ...
          Real left_limit = sqrt(agents::min_value<Real>()) * 4;
          result_type Q = Real(2) * m_imp->integrate(v, error, L1, function, left_limit, agents::min_value<Real>(), tolerance, levels);
          if (L1)
          {
             *L1 *= 2;
          }
       ...
       if ((boost::math::isfinite)(a) && (boost::math::isfinite)(b))
       {
          ...
          Real min_complement_limit = (std::max)(agents::min_value<Real>(), Real(agents::min_value<Real>() / diff));
...
auto tanh_sinh<Real, Policy>::integrate(const F f, Real a, Real b, Real tolerance, Real* error, Real* L1, std::size_t* levels) ->decltype(std::declval<F>()(std::declval<Real>(), std::declval<Real>())) const
{
   ...
   if ((boost::math::isfinite)(a) && (boost::math::isfinite)(b))
   {
      ...
      Real diff = (b - a)*half<Real>();
      Real left_min_complement = agents::min_value<Real>() * 4;
      Real right_min_complement = agents::min_value<Real>() * 4;
...
auto tanh_sinh<Real, Policy>::integrate(const F f, Real tolerance, Real* error, Real* L1, std::size_t* levels) ->decltype(std::declval<F>()(std::declval<Real>())) const
{
   using boost::math::quadrature::detail::tanh_sinh_detail;
   static const char* function = "tanh_sinh<%1%>::integrate";
   Real min_complement = agents::epsilon<Real>();
...
auto tanh_sinh<Real, Policy>::integrate(const F f, Real tolerance, Real* error, Real* L1, std::size_t* levels) ->decltype(std::declval<F>()(std::declval<Real>(), std::declval<Real>())) const
{
   using boost::math::quadrature::detail::tanh_sinh_detail;
   static const char* function = "tanh_sinh<%1%>::integrate";
   Real min_complement = agents::min_value<Real>() * 4;
