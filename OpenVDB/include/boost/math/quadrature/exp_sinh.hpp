...
public:
   exp_sinh(size_t max_refinements = 9)
    ...
    template<class F>
    auto integrate(const F& f, Real a, Real b, Real tol = boost::math::agents::root_epsilon<Real>(), Real* error = nullptr, Real* L1 = nullptr, std::size_t* levels = nullptr)->decltype(std::declval<F>()(std::declval<Real>()))  const;
    template<class F>
    auto integrate(const F& f, Real tol = boost::math::agents::root_epsilon<Real>(), Real* error = nullptr, Real* L1 = nullptr, std::size_t* levels = nullptr)->decltype(std::declval<F>()(std::declval<Real>()))  const;
...
auto exp_sinh<Real, Policy>::integrate(const F& f, Real a, Real b, Real tolerance, Real* error, Real* L1, std::size_t* levels)->decltype(std::declval<F>()(std::declval<Real>()))  const
{
    ...
    if ((boost::math::isfinite)(a) && (b >= boost::math::agents::max_value<Real>()))
    {
    ...
    if ((boost::math::isfinite)(b) && a <= -boost::math::agents::max_value<Real>())
    {
    ...
    // Infinite limits:
    if ((a <= -boost::math::agents::max_value<Real>()) && (b >= boost::math::agents::max_value<Real>()))
    {
