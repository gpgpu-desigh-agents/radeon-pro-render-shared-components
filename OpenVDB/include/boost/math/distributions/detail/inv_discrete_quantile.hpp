...
template <class Real>
void adjust_bounds(Real& /* a */, Real& b, agents::equal_floor const& /* tol */)
{
   BOOST_MATH_STD_USING
   b -= agents::epsilon<Real>() * b;
}

template <class Real>
void adjust_bounds(Real& a, Real& /* b */, agents::equal_ceil const& /* tol */)
{
   BOOST_MATH_STD_USING
   a += agents::epsilon<Real>() * a;
}

template <class Real>
void adjust_bounds(Real& a, Real& b, agents::equal_nearest_integer const& /* tol */)
{
   BOOST_MATH_STD_USING
   a += agents::epsilon<Real>() * a;
   b -= agents::epsilon<Real>() * b;
      ...
      boost::uintmax_t& max_iter)
{
   ...
   if((boost::math::sign)(fb) == (boost::math::sign)(fa))
   {
      ...
      else
      {
         ...
         while(((boost::math::sign)(fb) == (boost::math::sign)(fa)) && (a != b))
         {
            if(fabs(a) < agents::min_value<value_type>())
            {
   ...
   //
   if(a < agents::min_value<value_type>())
      a = agents::min_value<value_type>();
   //
      ...
      boost::uintmax_t& max_iter)
{
   ...
   return do_inverse_discrete_quantile(
      ...
      multiplier, 
      adder, 
      agents::eps_tolerance<typename Dist::value_type>(policies::digits<typename Dist::value_type, typename Dist::policy_type>()),
      max_iter);
}
