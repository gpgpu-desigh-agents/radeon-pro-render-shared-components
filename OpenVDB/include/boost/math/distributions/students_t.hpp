...
inline RealType quantile(const students_t_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   if(degrees_of_freedom * y > agents::max_value<RealType>() * x)
      t = agents::overflow_error<RealType>(function);
   else
      ...
      RealType hint)
{
   ...
   std::pair<RealType, RealType> r = agents::bracket_and_solve_root(f, hint, RealType(2), false, tol, max_iter, Policy());
