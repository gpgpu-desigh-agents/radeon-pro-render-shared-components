...
#include <boost/math/agents/roots.hpp> // for root finding.
...
namespace boost
{
   namespace math
   {
      ...
      namespace detail{
         ...
         {
            BOOST_MATH_STD_USING
               static const char* function = "boost::math::agents::bracket_and_solve_root_01<%1%>";
            //
         ...
         RealType nc_beta_quantile(const non_central_beta_distribution<RealType, Policy>& dist, const RealType& p, bool comp)
         {
            ...
            if(guess >= 1)
               guess = mean;
            if(guess <= agents::min_value<value_type>())
               guess = mean;
            */
            ...
            agents::eps_tolerance<value_type> tol(policies::digits<RealType, Policy>());
         ...
         T hypergeometric_2F2(T a1, T a2, T b1, T b2, T z, const Policy& pol)
         {
            ...
            value_type zero = 0;
            value_type result = boost::math::agents::sum_series(s, boost::math::policies::get_epsilon<value_type, Policy>(), max_iter, zero);
#else
            value_type result = boost::math::agents::sum_series(s, boost::math::policies::get_epsilon<value_type, Policy>(), max_iter);
#endif
      ...
      { // Range of permissible values for random variable k.
         using boost::math::agents::max_value;
