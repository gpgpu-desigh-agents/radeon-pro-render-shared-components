...
# pragma warning(disable:4389) // '==' : signed/unsigned mismatch in test_agents
#endif // _MSC_VER
...
bool check_probabilities(char const* function, std::vector<RealT> const& probabilities, RealT* presult, PolicyT const& pol)
{
    ...
    //
    if (fabs(sum - 1) > agents::epsilon<RealT>() * 2)
    {
        *presult = policies::raise_domain_error<RealT>(function,
...
std::pair<RealT,RealT> range(hyperexponential_distribution<RealT,PolicyT> const&)
{
    ...
    return std::make_pair(static_cast<RealT>(0), agents::max_value<RealT>()); // 0 to +<max value>
}
...
{
    return std::make_pair(agents::min_value<RealT>(), agents::max_value<RealT>()); // <min value> to +<max value>.
}
