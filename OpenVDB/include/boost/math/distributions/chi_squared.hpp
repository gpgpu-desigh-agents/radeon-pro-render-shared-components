...
{ // Range of supported values for random variable x.
   ...
   return std::pair<RealType, RealType>(static_cast<RealType>(0), agents::max_value<RealType>()); // 0 to + infinity.
}
...
//   RealType df = dist.degrees_of_freedom();
//   if(df <= 1)
//      return agents::domain_error<RealType>(
...
struct df_estimator
{
   ...
   {
      if(df <= agents::min_value<RealType>())
         return 1;
   ...
   RealType hint)
{
   ...
   std::pair<RealType, RealType> r =
     agents::bracket_and_solve_root(f, hint, RealType(2), false, tol, max_iter, Policy());
