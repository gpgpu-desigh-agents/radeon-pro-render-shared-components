...
#include <boost/math/agents/roots.hpp> // Newton-Raphson
...
namespace boost{ namespace math{
  ...
  inline RealType mode(const skew_normal_distribution<RealType, Policy>& dist)
  {
    ...
    result = agents::newton_raphson_iterate(detail::skew_normal_mode_functor<RealType, Policy>(helper), result,
  ...
  inline RealType quantile(const skew_normal_distribution<RealType, Policy>& dist, const RealType& p)
  {
    ...
    result = agents::newton_raphson_iterate(detail::skew_normal_quantile_functor<RealType, Policy>(dist, p), result,
      search_min, search_max, get_digits, m);
