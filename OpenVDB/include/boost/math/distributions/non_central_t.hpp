   ...
   namespace math
   {
      ...
      namespace detail{
         ...
         T non_central_t2_q(T v, T delta, T x, T y, const Policy& pol, T init_val)
         {
            ...
            // Starting Poisson weight:
            T pois;
            if((k < (int)(max_factorial<T>::value)) && (d2 < agents::log_max_value<T>()) && (log(d2) * k < agents::log_max_value<T>()))
            {
               //
     ...
     // now passed as function
            ...
            value_type guess = 0;
            if ( ((boost::math::isinf)(v)) || (v > 1 / boost::math::agents::epsilon<T>()) )
         ...
         T non_central_t_pdf(T n, T delta, T t, const Policy& pol)
         {
            ...
            T tol = agents::epsilon<T>() * result * 500;
         ...
         T mean(T v, T delta, const Policy& pol)
         {
            ...
            BOOST_MATH_STD_USING
            if (v > 1 / boost::math::agents::epsilon<T>() )
            {
              //normal_distribution<T, Policy> n(delta, 1);
            ...
            RealType delta, RealType x, RealType p, RealType q, const Policy& pol)
         {
            ...
            agents::eps_tolerance<RealType> tol(policies::digits<RealType, Policy>());
            boost::uintmax_t max_iter = policies::get_max_root_iterations<Policy>();
            //
            // Pick an initial guess:
            //
            RealType guess = 200;
            std::pair<RealType, RealType> ir = agents::bracket_and_solve_root(
