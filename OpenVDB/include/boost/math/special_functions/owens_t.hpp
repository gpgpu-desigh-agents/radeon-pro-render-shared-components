...
namespace boost
{
   namespace math
   {
      ...
      const unsigned short m = 20;
        ...
        inline RealType owens_t_T5(const RealType h, const RealType a, const Policy&)
        {
         ...
         std::pair<T, T> owens_t_T1_accelerated(T h, T a, const Policy& pol)
         {
            ...
            {
#endif
               n = itrunc(T(agents::log_max_value<T>() / 6));
#ifndef BOOST_NO_EXCEPTIONS
            }
            catch(...)
            {
            ...
            sum *= c;
            b = -n * n * b * 2;
            abs_err = ldexp(fabs(sum), -agents::digits<T>());
            ...
            while(j < n)
            {
               ...
               c = b - c;
               sum += c * term;
               abs_err += ldexp((std::max)(T(fabs(sum)), T(fabs(c*term))), -agents::digits<T>());
               b = (j + n) * (j - n) * b / ((j + T(0.5)) * (j + 1));
               ++j;
               //
               // Include an escape route to prevent calculating too many terms:
               //
               if((j > 10) && (fabs(sum * agents::epsilon<T>()) > fabs(c * term)))
                  break;
            }
            abs_err += fabs(c * term);
         ...
         inline std::pair<RealType, RealType> owens_t_T2_accelerated(const RealType h, const RealType a, const RealType ah, const Policy&)
         {
            ...
            {
#endif
               n = itrunc(RealType(agents::log_max_value<RealType>() / 6));
#ifndef BOOST_NO_EXCEPTIONS
            }
            catch(...)
            {
            ...
            for(int k = 0; k < n; ++k)
            {
               ...
               if(
                  ...
                  || (fabs(val) * agents::epsilon<RealType>() > fabs(c * s * z))  // Convergence!
         ...
         inline RealType owens_t_dispatch(const RealType h, const RealType a, const RealType ah, const Policy& pol, mpl::int_<64> const&)
         {
            ...
            if(a >= agents::max_value<RealType>())
            {
         ...
         inline RealType owens_t_dispatch(const RealType h, const RealType a, const RealType ah, const Policy& pol, const mpl::int_<65>&)
         {
            ...
            std::pair<RealType, RealType> p1(0, agents::max_value<RealType>()), p2(0, agents::max_value<RealType>());
      ...
      inline typename agents::promote_args<T1, T2>::type owens_t(T1 h, T2 a, const Policy& pol)
      {
      ...
      inline typename agents::promote_args<T1, T2>::type owens_t(T1 h, T2 a)
      {
