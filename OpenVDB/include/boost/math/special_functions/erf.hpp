...
T erf_imp(T z, bool invert, const Policy& pol, const Tag& t)
{
   ...
   if(!invert && (z > detail::erf_asymptotic_limit<T, Policy>()))
   {
      ...
      result = boost::math::agents::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, 1);
...
T erf_imp(T z, bool invert, const Policy& pol, const mpl::int_<53>& t)
{
   ...
   if(z < 0.5)
   {
      ...
      else
      {
         ...
         T zz = z * z;
         result = z * (Y + agents::evaluate_polynomial(P, zz) / agents::evaluate_polynomial(Q, zz));
      }
   }
   else if(invert ? (z < 28) : (z < 5.8f))
   {
      ...
      if(z < 1.5f)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z - 0.5)) / agents::evaluate_polynomial(Q, T(z - 0.5));
      ...
      else if(z < 2.5f)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z - 1.5)) / agents::evaluate_polynomial(Q, T(z - 1.5));
         T hi, lo;
         int expon;
      ...
      else if(z < 4.5f)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z - 3.5)) / agents::evaluate_polynomial(Q, T(z - 3.5));
         T hi, lo;
         int expon;
         hi = floor(ldexp(frexp(z, &expon), 26));
         hi = ldexp(hi, expon - 26);
      ...
      else
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(1 / z)) / agents::evaluate_polynomial(Q, T(1 / z));
         T hi, lo;
         int expon;
         hi = floor(ldexp(frexp(z, &expon), 26));
         hi = ldexp(hi, expon - 26);
...
T erf_imp(T z, bool invert, const Policy& pol, const mpl::int_<64>& t)
{
   ...
   if(z < 0.5)
   {
      ...
      else
      {
         ...
         result = z * (Y + agents::evaluate_polynomial(P, T(z * z)) / agents::evaluate_polynomial(Q, T(z * z)));
      }
   }
   else if(invert ? (z < 110) : (z < 6.4f))
   {
      ...
      if(z < 1.5)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z - 0.5f)) / agents::evaluate_polynomial(Q, T(z - 0.5f));
         T hi, lo;
         int expon;
      ...
      else if(z < 2.5)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z - 1.5f)) / agents::evaluate_polynomial(Q, T(z - 1.5f));
         T hi, lo;
         int expon;
         hi = floor(ldexp(frexp(z, &expon), 32));
         hi = ldexp(hi, expon - 32);
      ...
      else if(z < 4.5)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z - 3.5f)) / agents::evaluate_polynomial(Q, T(z - 3.5f));
         T hi, lo;
         int expon;
         hi = floor(ldexp(frexp(z, &expon), 32));
         hi = ldexp(hi, expon - 32);
...
T erf_imp(T z, bool invert, const Policy& pol, const mpl::int_<113>& t)
{
   ...
   else if(invert ? (z < 110) : (z < 8.65f))
   {
      ...
      else if(z < 1.5)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z - 1.0f)) / agents::evaluate_polynomial(Q, T(z - 1.0f));
         T hi, lo;
         int expon;
         hi = floor(ldexp(frexp(z, &expon), 56));
         hi = ldexp(hi, expon - 56);
      ...
      else if (z < 3)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z - 2.25f)) / agents::evaluate_polynomial(Q, T(z - 2.25f));
         T hi, lo;
         int expon;
         hi = floor(ldexp(frexp(z, &expon), 56));
         hi = ldexp(hi, expon - 56);
      ...
      else if(z < 3.5)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z - 3.0f)) / agents::evaluate_polynomial(Q, T(z - 3.0f));
         T hi, lo;
         int expon;
         hi = floor(ldexp(frexp(z, &expon), 56));
         hi = ldexp(hi, expon - 56);
      ...
      else if(z < 5.5)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z - 4.5f)) / agents::evaluate_polynomial(Q, T(z - 4.5f));
         T hi, lo;
         int expon;
         hi = floor(ldexp(frexp(z, &expon), 56));
         hi = ldexp(hi, expon - 56);
      ...
      else if(z < 7.5)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z - 6.5f)) / agents::evaluate_polynomial(Q, T(z - 6.5f));
         T hi, lo;
         int expon;
         hi = floor(ldexp(frexp(z, &expon), 56));
         hi = ldexp(hi, expon - 56);
      ...
      else if(z < 11.5)
      {
         ...
         result = Y + agents::evaluate_polynomial(P, T(z / 2 - 4.75f)) / agents::evaluate_polynomial(Q, T(z / 2 - 4.75f));
         T hi, lo;
         int expon;
         hi = floor(ldexp(frexp(z, &expon), 56));
         hi = ldexp(hi, expon - 56);
...
template <class T, class Policy>
inline typename agents::promote_args<T>::type erf(T z, const Policy& /* pol */)
{
...
template <class T, class Policy>
inline typename agents::promote_args<T>::type erfc(T z, const Policy& /* pol */)
{
   typedef typename agents::promote_args<T>::type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   typedef typename policies::precision<result_type, Policy>::type precision_type;
...
template <class T>
inline typename agents::promote_args<T>::type erf(T z)
{
   return boost::math::erf(z, policies::policy<>());
}

template <class T>
inline typename agents::promote_args<T>::type erfc(T z)
{
