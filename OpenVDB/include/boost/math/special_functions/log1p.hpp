...
{ // The function returns the natural logarithm of 1 + x.
   ...
   if(a < agents::epsilon<result_type>())
      return x;
   detail::log1p_series<result_type> s(x);
   boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
#if !BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582)) && !BOOST_WORKAROUND(__EDG_VERSION__, <= 245)
   result_type result = agents::sum_series(s, policies::get_epsilon<result_type, Policy>(), max_iter);
#else
   result_type zero = 0;
   result_type result = agents::sum_series(s, policies::get_epsilon<result_type, Policy>(), max_iter, zero);
#endif
...
{ // The function returns the natural logarithm of 1 + x.
   ...
   T result = 1 - x / 2 + agents::evaluate_polynomial(P, x) / agents::evaluate_polynomial(Q, x);
   result *= x;
...
template <class T, class Policy>
inline typename agents::promote_args<T>::type log1p(T x, const Policy&)
{ 
   typedef typename agents::promote_args<T>::type result_type;
...
template <class T>
inline typename agents::promote_args<T>::type log1p(T x)
{
   ...
   log1pmx(T x, const Policy& pol)
{
   ...
   T zero = 0;
   T result = boost::math::agents::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
   T result = boost::math::agents::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations<T>(function, max_iter, pol);
   return result;
}
...
template <class T>
inline typename agents::promote_args<T>::type log1pmx(T x)
{
