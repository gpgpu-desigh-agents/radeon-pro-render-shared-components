...
T expint_1_rational(const T& z, const mpl::int_<53>&)
{
   ...
   if(z <= 1)
   {
      ...
      result = agents::evaluate_polynomial(P, z) 
         / agents::evaluate_polynomial(Q, z);
      result += z - log(z) - Y;
   }
   else if(z < -boost::math::agents::log_min_value<T>())
   {
      ...
      T recip = 1 / z;
      result = 1 + agents::evaluate_polynomial(P, recip)
         / agents::evaluate_polynomial(Q, recip);
      result *= exp(-z) * recip;
   }
   else
   {
...
T expint_1_rational(const T& z, const mpl::int_<113>&)
{
   ...
   else if(z <= 4)
   {
      ...
      T recip = 1 / z;
      result = Y + agents::evaluate_polynomial(P, recip)
         / agents::evaluate_polynomial(Q, recip);
      result *= exp(-z) * recip;
   }
   else if(z < -boost::math::agents::log_min_value<T>())
...
inline T expint_as_fraction(unsigned n, T z, const Policy& pol)
{
   ...
   T result = agents::continued_fraction_b(
      f, 
...
inline T expint_as_series(unsigned n, T z, const Policy& pol)
{
   ...
   result = agents::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, result);
...
T expint_i_imp(T z, const Policy& pol, const mpl::int_<53>& tag)
{
   ...
   if(z <= 6)
   {
      ...
      T t = (z / 3) - 1;
      result = agents::evaluate_polynomial(P, t) 
         / agents::evaluate_polynomial(Q, t);
      t = (z - r1) - r2;
      result *= t;
      if(fabs(t) < 0.1)
      {
   ...
   else if (z <= 10)
   {
      ...
      T t = z / 2 - 4;
      result = Y + agents::evaluate_polynomial(P, t)
         / agents::evaluate_polynomial(Q, t);
      result *= exp(z) / z;
      result += z;
   }
   ...
   else
   {
      ...
      else
      {
         ...
         {
            result *= exp(z - 40) / z;
            if(result > agents::max_value<T>() / exp40)
            {
               result = policies::raise_overflow_error<T>(function, 0, pol);
            }
            else
