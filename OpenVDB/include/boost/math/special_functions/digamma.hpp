...
inline T digamma_imp_large(T x, const mpl::int_<113>*)
{
   ...
   result += 1 / (2 * x);
   T z = 1 / (x*x);
   result -= z * agents::evaluate_polynomial(P, z);
   return result;
}
//
...
T digamma_imp_1_2(T x, const mpl::int_<113>*)
{
   ...
   g -= root4;
   g -= root5;
   T r = agents::evaluate_polynomial(P, T(x-1)) / agents::evaluate_polynomial(Q, T(x-1));
   T result = g * Y + g * r;
...
T digamma_imp(T x, const mpl::int_<0>* t, const Policy& pol)
{
   ...
   //
   int lim = 10 + ((agents::digits<T>() - 50) * 240L) / 950;
   T two_x = ldexp(x, 1);
   if(x >= lim)
   {
