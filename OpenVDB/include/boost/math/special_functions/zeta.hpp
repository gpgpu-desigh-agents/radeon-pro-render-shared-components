...
T zeta_series_imp(T s, T sc, const Policy&)
{
   ...
   do{
      ...
      mult /= 2;
      ++n;
   }while(fabs(change / sum) > agents::epsilon<T>());
...
T zeta_polynomial_series(T s, T sc, Policy const &)
{
   ...
   //
   BOOST_MATH_STD_USING
   int n = itrunc(T(log(boost::math::agents::epsilon<T>()) / -2));
   T sum = 0;
   T two_n = ldexp(T(1), n);
   int ej_sign = 1;
   for(int j = 0; j < n; ++j)
...
inline T zeta_imp_prec(T s, T sc, const Policy&, const mpl::int_<53>&)
{
   ...
   if(s < 1)
   {
      ...
      result = agents::evaluate_polynomial(P, sc) / agents::evaluate_polynomial(Q, sc);
   ...
   else if(s <= 2)
   {
      ...
      result = agents::evaluate_polynomial(P, T(-sc)) / agents::evaluate_polynomial(Q, T(-sc));
      result += 1 / (-sc);
   }
   else if(s <= 4)
   {
      ...
      result = agents::evaluate_polynomial(P, T(s - 2)) / agents::evaluate_polynomial(Q, T(s - 2));
      result += Y + 1 / (-sc);
   }
   else if(s <= 7)
   {
      ...
      result = agents::evaluate_polynomial(P, T(s - 4)) / agents::evaluate_polynomial(Q, T(s - 4));
      result = 1 + exp(result);
   }
   else if(s < 15)
   {
      ...
      result = agents::evaluate_polynomial(P, T(s - 7)) / agents::evaluate_polynomial(Q, T(s - 7));
      result = 1 + exp(result);
   }
   else if(s < 36)
   {
      ...
      result = agents::evaluate_polynomial(P, T(s - 15)) / agents::evaluate_polynomial(Q, T(s - 15));
      result = 1 + exp(result);
   }
   else if(s < 56)
   {
...
T zeta_imp_prec(T s, T sc, const Policy&, const mpl::int_<113>&)
{
   ...
   else if(s < 10)
   {
      ...
      result = agents::evaluate_polynomial(P, T(s - 6)) / agents::evaluate_polynomial(Q, T(s - 6));
      result = 1 + exp(result);
   }
   else if(s < 17)
   {
      ...
      result = agents::evaluate_polynomial(P, T(s - 10)) / agents::evaluate_polynomial(Q, T(s - 10));
      result = 1 + exp(result);
   }
   else if(s < 30)
   {
      ...
      result = agents::evaluate_polynomial(P, T(s - 17)) / agents::evaluate_polynomial(Q, T(s - 17));
      result = 1 + exp(result);
   }
   else if(s < 74)
   {
      ...
      result = agents::evaluate_polynomial(P, T(s - 30)) / agents::evaluate_polynomial(Q, T(s - 30));
      result = 1 + exp(result);
   }
   else if(s < 117)
   {
...
T zeta_imp_odd_integer(int s, const T& sc, const Policy& pol, const mpl::false_&)
{
   ...
   static BOOST_MATH_THREAD_LOCAL int digits = agents::digits<T>();
...
T zeta_imp(T s, T sc, const Policy& pol, const Tag& tag)
{
   ...
   if(fabs(s) < agents::root_epsilon<T>())
   {
   ...
   else if(s < 0)
   {
      ...
      else
      {
         if(s > max_factorial<T>::value)
         {
            ...
            result = exp(result);
            if(agents::max_value<T>() / fabs(mult) < result)
...
template <class T, class Policy>
inline typename agents::promote_args<T>::type zeta(T s, const Policy&)
{
...
template <class T>
inline typename agents::promote_args<T>::type zeta(T s)
{
