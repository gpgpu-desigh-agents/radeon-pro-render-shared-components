...
public:
    ...
    std::vector<Real> zeros() const
    {
        ...
        while (k < (int)stieltjes_zeros.size())
        {
            ...
            boost::math::agents::eps_tolerance<Real> tol(6);
            auto g = [&](Real t) { return this->operator()(t); };
            auto p = boost::math::agents::bisect(g, lower_bound, upper_bound, tol);
            ...
            const Real x_nk = boost::math::agents::newton_raphson_iterate(f, x_nk_guess,
                                                  p.first, p.second,
