...
#include <boost/math/agents/series.hpp> // series functor.
//#include <boost/math/agents/polynomial.hpp>  // polynomial.
#include <boost/math/agents/rational.hpp>  // evaluate_polynomial.
#include <boost/mpl/int.hpp>
#include <boost/type_traits/is_integral.hpp>
#include <boost/math/agents/precision.hpp> // boost::math::agents::max_value().
#include <boost/math/agents/big_constant.hpp>
...
{
  BOOST_MATH_STD_USING
  static const T max_diff = boost::math::agents::root_epsilon<T>() * fabs(w_est);
...
inline T lambert_w0_small_z(T z, const Policy& pol, boost::mpl::int_<5> const&)
{
  ...
  {
    0, // z^0  Care: zeroth term needed by agents::evaluate_polynomial, but not in the Wolfram equation, so indexes are one different!
    1, // z^1 term.
    -1, // z^2 term
  ...
  using boost::math::agents::sum_series;
  using boost::math::agents::evaluate_polynomial;
  // http://www.boost.org/doc/libs/release/libs/math/doc/html/math_agentkit/roots/rational.html
...
T lambert_w_positive_rational_float(T z)
{
   ...
   if (z < 2)
   {
      ...
      { // 0.05 < z < 0.5
        ...
        // Maximum Relative Change in Control Points : 7.555e-04 Y offset : -8.196592331e-01
         ...
         return z * (Y + boost::math::agents::evaluate_polynomial(P, z) / boost::math::agents::evaluate_polynomial(Q, z));
      }
      else
      { // 0.5 < z < 2
        // Max error in interpolated form: 1.018e-08
         ...
         return z * (Y + boost::math::agents::evaluate_rational(P, Q, z));
      }
   }
   else if (z < 6)
   {
      ...
      return Y + boost::math::agents::evaluate_rational(P, Q, z);
   }
   else if (z < 18)
   {
      // 6 < z < 18
   ...
   else if (z < 9897.12905874)  // 2.8 < log(z) < 9.2
   {
      ...
      T log_w = log(z);
      return log_w + Y + boost::math::agents::evaluate_polynomial(P, log_w) / boost::math::agents::evaluate_polynomial(Q, log_w);
   }
...
T lambert_w_negative_rational_float(T z, const Policy& pol)
{
   ...
   { // Very close to branch singularity.
     // Max error in interpolated form: 5.269e-08
      ...
      return -d / (Y + boost::math::agents::evaluate_polynomial(P, d) / boost::math::agents::evaluate_polynomial(Q, d));
   }
   else
   {
...
T lambert_w_positive_rational_double(T z)
{
   ...
   if (z < 2)
   {
      if (z < 0.5)
      {
         ...
         return z * (offset + boost::math::agents::evaluate_polynomial(P, z) / boost::math::agents::evaluate_polynomial(Q, z));
      }
      else
      {
         ...
         return z * (offset + boost::math::agents::evaluate_rational(P, Q, z));
      }
   }
   else if (z < 6)
   {
   ...
   else if (z < 18)
   {
      ...
      return offset + boost::math::agents::evaluate_rational(P, Q, z);
   }
   else if (z < 9897.12905874)  // 2.8 < log(z) < 9.2
   {
      ...
      T log_w = log(z);
      return log_w + Y + boost::math::agents::evaluate_rational(P, Q, log_w);
   }
   else if (z < 7.896296e+13)  // 9.2 < log(z) <= 32
   {
...
T lambert_wm1_imp(const T z, const Policy&  pol)
{
  ...
  if (z > -boost::math::agents::min_value<T>())
  ...
  { // Close to singularity/branch point z = -0.3678794411714423215955237701614608727 but on W-1 branch.
    ...
    if (p2 > 0)
    {
      ...
      if (boost::math::agents::digits<T>() > 53)
...
//! Lambert W0 using User-defined policy.
  template <class T, class Policy>
  inline
    typename boost::math::agents::promote_args<T>::type
  ...
  lambert_w0_prime(T z, const Policy& pol)
  {
    ...
    {
        return numeric_limits<result_type>::has_infinity ? numeric_limits<result_type>::infinity() : boost::math::agents::max_value<result_type>();
    }
