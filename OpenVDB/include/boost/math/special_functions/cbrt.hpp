...
T cbrt_imp(T z, const Policy& pol)
{
   ...
   guess = agents::evaluate_polynomial(P, guess);
   int i_exp3 = i_exp / 3;
   ...
   // We do this here rather than calling agents::halley_iterate since we can
   ...
   //
   T eps = (new_prec::value > 3) ? policies::get_epsilon<T, new_policy>() : ldexp(T(1), -2 - agents::digits<T>() / 3);
   T diff;
...
template <class T, class Policy>
inline typename agents::promote_args<T>::type cbrt(T z, const Policy& pol)
{
...
template <class T>
inline typename agents::promote_args<T>::type cbrt(T z)
{
