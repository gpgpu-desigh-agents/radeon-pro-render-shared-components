...
T expm1_imp(T x, const mpl::int_<0>&, const Policy& pol)
{
   ...
   T result = agents::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter);
#else
   T zero = 0;
   T result = agents::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, zero);
#endif
...
T expm1_imp(T x, const mpl::int_<53>&, const P& pol)
{
   ...
   T result = x * Y + x * agents::evaluate_polynomial(n, x) / agents::evaluate_polynomial(d, x);
   return result;
}
...
template <class T, class Policy>
inline typename agents::promote_args<T>::type expm1(T x, const Policy& /* pol */)
{
...
template <class T>
inline typename agents::promote_args<T>::type expm1(T x)
{
