...
namespace boost{
   namespace math{
      ...
      typename boost::math::agents::promote_args<T,U>::type relative_difference(const T& arg_a, const U& arg_b)
      {
         typedef typename boost::math::agents::promote_args<T, U>::type result_type;
         result_type a = arg_a;
         result_type b = arg_b;
         ...
         agents::min_value<result_type>(),
         static_cast<result_type>((std::numeric_limits<double>::min)()));
         result_type max_val = (std::min)(
            agents::max_value<result_type>(),
            static_cast<result_type>((std::numeric_limits<double>::max)()));
#else
         result_type min_val = agents::min_value<result_type>();
         result_type max_val = agents::max_value<result_type>();
#endif
      ...
      template <>
      inline boost::math::agents::promote_args<double, double>::type relative_difference(const double& arg_a, const double& arg_b)
      {
         BOOST_MATH_STD_USING
         double a = arg_a;
         double b = arg_b;
         ...
         //
         double min_val = (std::max)((double)agents::min_value<long double>(), agents::min_value<double>());
         double max_val = (std::min)((double)agents::max_value<long double>(), agents::max_value<double>());
      ...
      template <class T, class U>
      inline typename boost::math::agents::promote_args<T, U>::type epsilon_difference(const T& arg_a, const U& arg_b)
      {
         typedef typename boost::math::agents::promote_args<T, U>::type result_type;
         result_type r = relative_difference(arg_a, arg_b);
         if(agents::max_value<result_type>() * boost::math::agents::epsilon<result_type>() < r)
            return agents::max_value<result_type>();
         return r / boost::math::agents::epsilon<result_type>();
      }
} // namespace math
} // namespace boost
