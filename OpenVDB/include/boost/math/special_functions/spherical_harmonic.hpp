...
T spherical_harmonic_r(unsigned n, int m, T theta, T phi, const Policy& pol)
{
   ...
   if(m&1)
   {
      ...
      T mod = boost::math::agents::fmod_workaround(theta, T(2 * constants::pi<T>()));
      if(mod < 0)
...
std::complex<T> spherical_harmonic(unsigned n, int m, U theta, U phi, const Policy& pol)
{
   ...
   {
      // Check phase if theta is outside [0, PI]:
      U mod = boost::math::agents::fmod_workaround(theta, U(2 * constants::pi<U>()));
      if(mod < 0)
...
inline std::complex<typename agents::promote_args<T1, T2>::type> 
