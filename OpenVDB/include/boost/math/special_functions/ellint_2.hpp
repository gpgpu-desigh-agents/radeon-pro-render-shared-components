...
#include <boost/math/agents/workaround.hpp>
...
T ellint_e_imp(T phi, T k, const Policy& pol)
{
    ...
    else
    {
       ...
       //
       T rphi = boost::math::agents::fmod_workaround(phi, T(constants::half_pi<T>()));
       T m = boost::math::round((phi - rphi) / constants::half_pi<T>());
       int s = 1;
       if(boost::math::agents::fmod_workaround(m, T(2)) > 0.5)
       {
          m += 1;
          s = -1;
       ...
       T k2 = k * k;
       if(boost::math::pow<3>(rphi) * k2 / 6 < agents::epsilon<T>() * fabs(rphi))
       {
...
template <typename T, typename Policy>
inline typename agents::promote_args<T>::type ellint_2(T k, const Policy& pol, const mpl::true_&)
{
...
inline typename agents::promote_args<T1, T2>::type ellint_2(T1 k, T2 phi, const mpl::false_&)
{
...
// Complete elliptic integral (Legendre form) of the second kind
template <typename T>
inline typename agents::promote_args<T>::type ellint_2(T k)
{
...
// Elliptic integral (Legendre form) of the second kind
template <class T1, class T2>
inline typename agents::promote_args<T1, T2>::type ellint_2(T1 k, T2 phi)
{
...
template <class T1, class T2, class Policy>
inline typename agents::promote_args<T1, T2>::type ellint_2(T1 k, T2 phi, const Policy& pol)
{
