...
inline T get_smallest_value(mpl::true_ const&)
{
   ...
   return (_mm_getcsr() & (_MM_FLUSH_ZERO_ON | 0x40)) ? agents::min_value<T>() : m;;
#else
   return ((agents::min_value<T>() / 2) == 0) ? agents::min_value<T>() : m;
#endif
}
...
inline T calc_min_shifted(const mpl::true_&)
{
   ...
   return ldexp(agents::min_value<T>(), agents::digits<T>() + 1);
}
template <class T>
inline T calc_min_shifted(const mpl::false_&)
{
   ...
   return scalbn(agents::min_value<T>(), std::numeric_limits<T>::digits + 1);
}
...
T float_next_imp(const T& val, const mpl::true_&, const Policy& pol)
{
   ...
   {
      if(val < 0)
         return -agents::max_value<T>();
      return policies::raise_domain_error<T>(
         function,
         "Argument must be finite, but got %1%", val, pol);
   }

   if(val >= agents::max_value<T>())
      return policies::raise_overflow_error<T>(function, 0, pol);

   if(val == 0)
      return detail::get_smallest_value<T>();

   if((fpclass != (int)FP_SUBNORMAL) && (fpclass != (int)FP_ZERO) && (fabs(val) < detail::get_min_shift_value<T>()) && (val != -agents::min_value<T>()))
   {
      //
      ...
      //
      return ldexp(float_next(T(ldexp(val, 2 * agents::digits<T>())), pol), -2 * agents::digits<T>());
   }

   if(-0.5f == frexp(val, &expon))
      --expon; // reduce exponent when val is a power of two, and negative.
   T diff = ldexp(T(1), expon - agents::digits<T>());
   if(diff == 0)
...
template <class T, class Policy>
inline typename agents::promote_args<T>::type float_next(const T& val, const Policy& pol)
{
...
inline double float_next(const double& val, const Policy& pol)
{
   ...
   if(val >= agents::max_value<double>())
      return policies::raise_overflow_error<double>(function, 0, pol);

   return ::_nextafter(val, agents::max_value<double>());
}
#endif

template <class T>
inline typename agents::promote_args<T>::type float_next(const T& val)
{
...
T float_prior_imp(const T& val, const mpl::true_&, const Policy& pol)
{
   ...
   if((fpclass != (int)FP_SUBNORMAL) && (fpclass != (int)FP_ZERO) && (fabs(val) < detail::get_min_shift_value<T>()) && (val != agents::min_value<T>()))
   {
      ...
      // This avoids issues with the Intel SSE2 registers when the FTZ or DAZ flags are set.
      //
      return ldexp(float_prior(T(ldexp(val, 2 * agents::digits<T>())), pol), -2 * agents::digits<T>());
   }
...
template <class T, class Policy>
inline typename agents::promote_args<T>::type float_prior(const T& val, const Policy& pol)
{
   typedef typename agents::promote_args<T>::type result_type;
...
template <class T>
inline typename agents::promote_args<T>::type float_prior(const T& val)
{
   return float_prior(val, policies::policy<>());
}

template <class T, class U, class Policy>
inline typename agents::promote_args<T, U>::type nextafter(const T& val, const U& direction, const Policy& pol)
{
...
inline typename agents::promote_args<T, U>::type nextafter(const T& val, const U& direction)
{
...
T float_distance_imp(const T& a, const T& b, const mpl::true_&, const Policy& pol)
{
   ...
   // because we actually have fewer than agents::digits<T>()
   // significant bits in the representation:
   //
   (void)frexp(((boost::math::fpclassify)(a) == (int)FP_SUBNORMAL) ? agents::min_value<T>() : a, &expon);
   ...
   if(b > upper)
   {
      ...
      result += (expon2 - expon - 1) * ldexp(T(1), agents::digits<T>() - 1);
   }
   //
   // Use compensated double-double addition to avoid rounding
   // errors in the subtraction:
   //
   expon = agents::digits<T>() - expon;
   T mb, x, y, z;
   if(((boost::math::fpclassify)(a) == (int)FP_SUBNORMAL) || (b - a < agents::min_value<T>()))
   {
      //
      ...
      //
      T a2 = ldexp(a, agents::digits<T>());
      T b2 = ldexp(b, agents::digits<T>());
      mb = -(std::min)(T(ldexp(upper, agents::digits<T>())), b2);
      x = a2 + mb;
      z = x - a2;
      y = (a2 - (x - z)) + (mb - z);

      expon -= agents::digits<T>();
   }
   else
   {
      mb = -(std::min)(upper, b);
...
T float_distance_imp(const T& a, const T& b, const mpl::false_&, const Policy& pol)
{
   ...
   // significant bits in the representation:
   //
   expon = 1 + ilogb(((boost::math::fpclassify)(a) == (int)FP_SUBNORMAL) ? agents::min_value<T>() : a);
...
template <class T, class U, class Policy>
inline typename agents::promote_args<T, U>::type float_distance(const T& a, const U& b, const Policy& pol)
{
   typedef typename agents::promote_args<T, U>::type result_type;
...
template <class T, class U>
typename agents::promote_args<T, U>::type float_distance(const T& a, const U& b)
{
...
T float_advance_imp(T val, int distance, const mpl::true_&, const Policy& pol)
{
   ...
   if(val <= agents::min_value<T>())
   {
      limit = sign(T(distance)) * agents::min_value<T>();
   }
   ...
   T diff = 0;
   if(val != 0)
      diff = distance * ldexp(T(1), expon - agents::digits<T>());
   if(diff == 0)
...
template <class T, class Policy>
inline typename agents::promote_args<T>::type float_advance(T val, int distance, const Policy& pol)
{
   typedef typename agents::promote_args<T>::type result_type;
...
template <class T>
inline typename agents::promote_args<T>::type float_advance(const T& val, int distance)
{
