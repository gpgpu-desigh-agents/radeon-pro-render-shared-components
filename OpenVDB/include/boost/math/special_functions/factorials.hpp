...
inline float factorial<float>(unsigned i)
{
   ...
   return agents::overflow_error<float>(BOOST_CURRENT_FUNCTION);
}
...
inline double factorial<double>(unsigned i)
{
   ...
   return agents::overflow_error<double>(BOOST_CURRENT_FUNCTION);
}
*/
template <class T, class Policy>
T double_factorial(unsigned i, const Policy& pol)
{
   ...
   if(i & 1)
   {
      ...
      if(ldexp(agents::max_value<T>(), -static_cast<int>(i+1) / 2) > result)
   ...
   else
   {
      ...
      if(ldexp(agents::max_value<T>(), -(int)n) > result)
...
inline T falling_factorial_imp(T x, unsigned n, const Policy& pol)
{
   ...
   if(x < 0.5f)
   {
      ...
      if(n > max_factorial<T>::value - 2)
      {
         ...
         if(agents::max_value<T>() / fabs(t1) < fabs(t2))
...
template <class RT>
inline typename agents::promote_args<RT>::type
   falling_factorial(RT x, unsigned n)
{
   typedef typename agents::promote_args<RT>::type result_type;
