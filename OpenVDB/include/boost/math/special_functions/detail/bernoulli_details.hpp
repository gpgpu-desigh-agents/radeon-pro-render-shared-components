...
#include <boost/math/agents/atomic.hpp>
...
T b2n_asymptotic(int n)
{
   ...
   return ((n / 2) & 1 ? 1 : -1) * (approximate_log_of_bernoulli_bn > agents::log_max_value<T>() 
...
inline std::size_t find_bernoulli_overflow_limit(const mpl::false_&)
{
   ...
   ulong_long_type t = lltrunc(boost::math::agents::log_max_value<T>());
   max_bernoulli_root_functor fun(t);
   boost::math::agents::equal_floor tol;
   boost::uintmax_t max_iter = boost::math::policies::get_max_root_iterations<Policy>();
   double result = boost::math::agents::toms748_solve(fun, sqrt(double(t)), double(t), tol, max_iter).first / 2;
...
public:
   bernoulli_numbers_cache() : m_overflow_limit((std::numeric_limits<std::size_t>::max)())
      ...
      , m_counter(0)
#endif
      , m_current_precision(boost::math::agents::digits<T>())
   {}
   ...
   void tangent(std::size_t m)
   {
      ...
      for(std::size_t i = std::max<size_t>(2, prev_size); i < m; i++)
      {
         ...
         if(i >= min_overflow_index && (boost::math::agents::max_value<T>() / (i-1) < m_intermediates[1]) )
         {
            std::fill(tn.begin() + i, tn.end(), boost::math::agents::max_value<T>());
            break;
         }
         ...
         for(std::size_t j = 2; j <= i; j++)
         {
            overflow_check =
                  ...
                  (boost::math::agents::max_value<T>() / (i - j) < m_intermediates[j])
                  || (boost::math::agents::max_value<T>() / (i - j + 2) < m_intermediates[j-1])
                  || (boost::math::agents::max_value<T>() - m_intermediates[j] * (i - j) < m_intermediates[j-1] * (i - j + 2))
   ...
   void tangent_numbers_series(const std::size_t m)
   {
      ...
      for(std::size_t i = old_size; i < m; i++)
      {
         ...
         b /= (power_two - 1);
         bool overflow_check = (i >= min_overflow_index) && (agents::max_value<T>() / tn[static_cast<typename container_type::size_type>(i)] < b);
         if(overflow_check)
         {
            m_overflow_limit = i;
            while(i < m)
            {
               b = std::numeric_limits<T>::has_infinity ? std::numeric_limits<T>::infinity() : agents::max_value<T>();
   ...
   OutputIterator copy_bernoulli_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      //
      if(m_current_precision < boost::math::agents::digits<T>())
      {
         bn.clear();
         tn.clear();
      ...
      if((static_cast<std::size_t>(m_counter.load(BOOST_MATH_ATOMIC_NS::memory_order_consume)) < start + n)
         || (static_cast<int>(m_current_precision.load(BOOST_MATH_ATOMIC_NS::memory_order_consume)) < boost::math::agents::digits<T>()))
      {
         boost::detail::lightweight_mutex::scoped_lock l(m_mutex);
         ...
         if((static_cast<std::size_t>(m_counter.load(BOOST_MATH_ATOMIC_NS::memory_order_consume)) < start + n)
            || (static_cast<int>(m_current_precision.load(BOOST_MATH_ATOMIC_NS::memory_order_consume)) < boost::math::agents::digits<T>()))
         {
            if(static_cast<int>(m_current_precision.load(BOOST_MATH_ATOMIC_NS::memory_order_consume)) < boost::math::agents::digits<T>())
            {
               bn.clear();
               tn.clear();
               m_intermediates.clear();
   ...
   OutputIterator copy_tangent_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      for(std::size_t i = start; i < start + n; ++i)
      {
         ...
         {
            if(agents::max_value<T>() * tangent_scale_factor<T>() < tn[static_cast<typename container_type::size_type>(i)])
               *out = policies::raise_overflow_error<T>("boost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol);
            else
