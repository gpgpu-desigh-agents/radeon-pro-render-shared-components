...
T ibeta_inv_ab_imp(const T& b, const T& z, const T& p, const T& q, bool swap_ab, const Policy& pol)
{
   ...
   if(p == 0)
   {
      return swap_ab ? agents::min_value<T>() : agents::max_value<T>();
   }
   if(q == 0)
   {
      return swap_ab ? agents::max_value<T>() : agents::min_value<T>();
   }
   //
   ...
   //
   agents::eps_tolerance<T> tol(policies::digits<T, Policy>());
   //
   ...
   if(guess < 10)
   {
      ...
      factor = (v < sqrt(agents::epsilon<T>())) ? 2 : (guess < 20 ? 1.2f : 1.1f);
      ...
      ibeta_inva(RT1 b, RT2 x, RT3 p, const Policy& pol)
{
   ...
   if(p == 1)
   {
      return agents::min_value<result_type>();
   }
