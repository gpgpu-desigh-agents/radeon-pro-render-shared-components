...
namespace boost { namespace math { namespace detail{
   ...
   T bessel_k1_imp(const T& x, const mpl::int_<24>&)
   {
      ...
      if(x <= 1)
      {
         ...
         T a = x * x / 4;
         a = ((agents::evaluate_rational(P, Q, a) + Y) * a * a + a / 2 + 1) * x / 2;
         ...
         return agents::evaluate_polynomial(P2, T(x * x)) * x + 1 / x + log(x) * a;
      }
      else
      {
         ...
         if(x < agents::log_max_value<T>())
            return ((agents::evaluate_rational(P, Q, T(1 / x)) + Y) * exp(-x) / sqrt(x));
         else
         {
            T ex = exp(-x / 2);
            return ((agents::evaluate_rational(P, Q, T(1 / x)) + Y) * ex / sqrt(x)) * ex;
         }
      }
   }
   ...
   T bessel_k1_imp(const T& x, const mpl::int_<53>&)
   {
      ...
      if(x <= 1)
      {
         ...
         return agents::evaluate_rational(P2, Q2, T(x * x)) * x + 1 / x + log(x) * a;
      }
      else
      {
   ...
   T bessel_k1_imp(const T& x, const mpl::int_<64>&)
   {
      ...
      else
      {
         ...
         if(x < agents::log_max_value<T>())
            return ((agents::evaluate_polynomial(P, T(1 / x)) / agents::evaluate_polynomial(Q, T(1 / x)) + Y) * exp(-x) / sqrt(x));
         else
         {
            T ex = exp(-x / 2);
            return ((agents::evaluate_polynomial(P, T(1 / x)) / agents::evaluate_polynomial(Q, T(1 / x)) + Y) * ex / sqrt(x)) * ex;
         }
      }
   }
   ...
   T bessel_k1_imp(const T& x, const mpl::int_<113>&)
   {
    ...
    {
       if(boost::math::agents::digits<T>() <= 24)
          return bessel_k1_imp(x, mpl::int_<24>());
       else if(boost::math::agents::digits<T>() <= 53)
          return bessel_k1_imp(x, mpl::int_<53>());
       else if(boost::math::agents::digits<T>() <= 64)
          return bessel_k1_imp(x, mpl::int_<64>());
       else if(boost::math::agents::digits<T>() <= 113)
