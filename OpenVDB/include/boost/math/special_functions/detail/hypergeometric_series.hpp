...
#include <boost/math/policies/error_handling.hpp>
  ...
  inline T sum_pFq_series(detail::hypergeometric_pFq_generic_series_term<T, p, q>& term, const Policy& pol)
  {
    ...
    const T zero = 0;
    const T result = boost::math::agents::sum_series(term, boost::math::policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
    const T result = boost::math::agents::sum_series(term, boost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
  ...
  {
     BOOST_MATH_STD_USING
     T sum(0), term(1), upper_limit(sqrt(boost::math::agents::max_value<T>())), diff;
     T lower_limit(1 / upper_limit);
     unsigned n = 0;
     int log_scaling_factor = itrunc(boost::math::agents::log_max_value<T>()) - 2;
