...
namespace boost { namespace math {
   ...
   namespace detail {
      ...
      int temme_jy(T v, T x, T* Y, T* Y1, const Policy& pol)
      {
         ...
         e = abs(v) < agents::epsilon<T>() ? T(v*pi<T>()*pi<T>() / 2)
            : T(2 * spv2 * spv2 / v);

         T g1 = (v == 0) ? T(-euler<T>()) : T((gp - gm) / ((1 + gp) * (1 + gm) * 2 * v));
         T g2 = (2 + gp + gm) / ((1 + gp) * (1 + gm) * 2);
         T vspv = (fabs(v) < agents::epsilon<T>()) ? T(1/constants::pi<T>()) : T(v / spv);
      ...
      int bessel_jy(T v, T x, T* J, T* Y, int kind, const Policy& pol)
      {
         ...
         else                                    // x in (2, \infty)
         {
            ...
            T init = sqrt(agents::min_value<T>());
            ...
            else
            {
               ...
               {
                  T t = 2 * (u + k) / x;
                  if((t > 1) && (agents::max_value<T>() / t < current))
                  {
                     over = true;
                     break;
                  }
         ...
         if (reflect)
         {
            if((sp != 0) && (agents::max_value<T>() * fabs(Yv_scale) < fabs(sp * Yv)))
            ...
            if((cp != 0) && (agents::max_value<T>() * fabs(Yv_scale) < fabs(cp * Yv)))
         ...
         {
            *J = Jv;
            if(agents::max_value<T>() * fabs(Yv_scale) < fabs(Yv))
