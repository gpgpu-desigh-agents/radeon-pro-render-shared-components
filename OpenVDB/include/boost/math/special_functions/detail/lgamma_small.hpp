...
T lgamma_small_imp(T z, T zm1, T zm2, const mpl::int_<64>&, const Policy& /* l */, const Lanczos&)
{
   ...
   else if(z > 2)
   {
      ...
      T r = zm2 * (z + 1);
      T R = agents::evaluate_polynomial(P, zm2);
      R /= agents::evaluate_polynomial(Q, zm2);
   ...
   else
   {
      ...
      if(z <= 1.5)
      {
         ...
         T r = agents::evaluate_polynomial(P, zm1) / agents::evaluate_polynomial(Q, zm1);
         T prefix = zm1 * zm2;
      ...
      else
      {
         ...
         T r = zm2 * zm1;
         T R = agents::evaluate_polynomial(P, T(-zm2)) / agents::evaluate_polynomial(Q, T(-zm2));
...
T lgamma_small_imp(T z, T zm1, T zm2, const mpl::int_<113>&, const Policy& /* l */, const Lanczos&)
{
   ...
   else
   {
      ...
      else if(z <= 1.625)
      {
         ...
         T r = zm2 * zm1;
         T R = agents::evaluate_polynomial(P, T(0.625 - zm1)) / agents::evaluate_polynomial(Q, T(0.625 - zm1));
