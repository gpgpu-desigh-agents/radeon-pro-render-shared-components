  ...
  namespace boost { namespace math { namespace detail {
     ...
     struct hypergeometric_1F1_igamma_series
     {
           ...
           : delta_poch(-delta), alpha_poch(alpha), x(x), k(0), cache_offset(0), pol(pol)
        {
           ...
           log_scaling = itrunc(log_term - 3 - boost::math::agents::log_min_value<T>() / 50);
     ...
     T hypergeometric_1F1_shift_on_a(T h, const T& a_local, const T& b_local, const T& x, int a_shift, const Policy& pol, int& log_scaling)
     {
        ...
        else if (a_shift > 0)
        {
           ...
           if (crossover_shift > 1)
           {
              ...
              boost::uintmax_t max_iter = boost::math::policies::get_max_series_iterations<Policy>();
              T b_ratio = boost::math::agents::function_ratio_from_backwards_recurrence(b_coef, boost::math::policies::get_epsilon<T, Policy>(), max_iter);
              ...
              T comparitor = boost::math::agents::apply_recurrence_relation_backward(a_coef, crossover_shift, second, first, &backwards_scale);
              log_scaling -= backwards_scale;
              if ((h < 1) && (agents::max_value<T>() * h > comparitor))
              {
                 // Need to rescale!
              ...
              if (crossover_shift < a_shift)
              {
                 ...
                 h = boost::math::agents::apply_recurrence_relation_forward(a_coef_2, a_shift - crossover_shift - 1, first, second, &log_scaling);
              }
              else
                 h = first;
           }
           else
           {
              ...
              h = boost::math::agents::apply_recurrence_relation_forward(a_coef, --a_shift, h, second, &log_scaling);
           }
        }
        else
        {
           ...
           else
           {
              ...
              T comparitor = boost::math::agents::apply_recurrence_relation_forward(a_coef, -(a_shift + 1), first, second);
              if (boost::math::agents::min_value<T>() * comparitor > h)
              {
     ...
     T hypergeometric_1F1_shift_on_b(T h, const T& a, const T& b_local, const T& x, int b_shift, const Policy& pol, int& log_scaling)
     {
        ...
        else if (b_shift > 0)
        {
           ...
           T first = 1;  // arbitrary value;
           T second = 1 / boost::math::agents::function_ratio_from_backwards_recurrence(b_coef, boost::math::policies::get_epsilon<T, Policy>(), max_iter);
           boost::math::policies::check_series_iterations<T>("boost::math::hypergeometric_1F1_large_abz<%1%>(%1%,%1%,%1%)", max_iter, pol);
           if (b_shift == 1)
              h = h / second;
           else
           {
              ...
              int local_scale = 0;
              T comparitor = boost::math::agents::apply_recurrence_relation_backward(b_coef_2, --b_shift, first, second, &local_scale);
        ...
        else
        {
           ...
           else
           {
              ...
              boost::math::detail::hypergeometric_1F1_recurrence_b_coefficients<T> b_coef(a, b_local, x);
              boost::uintmax_t max_iter = boost::math::policies::get_max_series_iterations<Policy>();
              second = h / boost::math::agents::function_ratio_from_backwards_recurrence(b_coef, boost::math::policies::get_epsilon<T, Policy>(), max_iter);
              boost::math::policies::check_series_iterations<T>("boost::math::hypergeometric_1F1_large_abz<%1%>(%1%,%1%,%1%)", max_iter, pol);
           }
           if (b_shift == -1)
              h = second;
           else
           {
              ...
              h = boost::math::agents::apply_recurrence_relation_backward(b_coef_2, -(++b_shift), h, second, &log_scaling);
           }
        }
        return h;
     }
     ...
     T hypergeometric_1F1_large_series(const T& a, const T& b, const T& z, const Policy& pol, int& log_scaling)
     {
        ...
        if (a_shift && (a_local == 0))
        {
           ...
           h = boost::math::agents::apply_recurrence_relation_forward(coef, a_shift - 1, h, h2, &log_scaling);
     ...
     T hypergeometric_1F1_large_abz(const T& a, const T& b, const T& z, const Policy& pol, int& log_scaling)
     {
        ...
        //
        cost = 50 + fabs(b - a);
        if((b > 1) && (cost <= current_cost) && (z < agents::log_max_value<T>()) && (z < 11356) && (b - a != 0.5f))
        {
