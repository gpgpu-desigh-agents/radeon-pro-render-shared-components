...
#include <boost/math/special_functions/bessel_iterators.hpp>
  ...
  namespace boost { namespace math { namespace detail {
     ...
     bool hypergeometric_1F1_is_tricomi_viable_positive_b(const T& a, const T& b, const T& z)
     {
        ...
        T v = b - 1;
        return log(boost::math::constants::e<T>() * x / (2 * v)) * v > agents::log_min_value<T>();
     }
     ...
     {
        using std::fabs;
        return (agents::min_value<T>() / agents::epsilon<T>()) * (fabs(target) > 1 ? target : 1);
     }
     ...
     struct hypergeometric_1F1_AS_13_3_7_tricomi_series
     {
           ...
           two_a_minus_b(2 * a - b), pol(pol_), n(2)
        {
           ...
           if (fabs(bessel_cache[cache_size - 1]) < agents::min_value<T>() / agents::epsilon<T>())
           {
           ...
           if ((term * bessel_cache[cache_size - 1] < agents::min_value<T>() / (agents::epsilon<T>() * agents::epsilon<T>())) || !(boost::math::isfinite)(term) || (!std::numeric_limits<T>::has_infinity && (fabs(term) > agents::max_value<T>())))
           {
           ...
           if constexpr (std::numeric_limits<T>::has_infinity)
           {
              ...
              if ((boost::math::isnan)(bessel_cache[cache_size - 1]) || (fabs(bessel_cache[cache_size - 1]) >= agents::max_value<T>()))
                 policies::raise_evaluation_error("hypergeometric_1F1_AS_13_3_7_tricomi_series<%1%>", "Expected finite Bessel function result but got %1%", bessel_cache[cache_size - 1], pol);
#else
           if ((std::numeric_limits<T>::has_infinity && !(boost::math::isfinite)(bessel_cache[cache_size - 1])) 
              || (!std::numeric_limits<T>::has_infinity && ((boost::math::isnan)(bessel_cache[cache_size - 1]) || (fabs(bessel_cache[cache_size - 1]) >= agents::max_value<T>()))))
              policies::raise_evaluation_error("hypergeometric_1F1_AS_13_3_7_tricomi_series<%1%>", "Expected finite Bessel function result but got %1%", bessel_cache[cache_size - 1], pol);
#endif
           ...
           if (bessel_arg > 0)
           {
              ...
              if (b_minus_1_plus_n > 0)
              {
                 ...
                 for (int j = cache_size - 1; j >= 0; --j, ++i)
                 {
                    ...
                    //
                    if ((j < cache_size - 2) && (agents::max_value<T>() / fabs(64 * bessel_cache[j] / bessel_cache[j + 1]) < fabs(bessel_cache[j])))
                    {
                       T rescale = pow(fabs(bessel_cache[j] / bessel_cache[j + 1]), j + 1) * 2;
                       if (!((boost::math::isfinite)(rescale)))
                          rescale = agents::max_value<T>();
     ...
     T hypergeometric_1F1_AS_13_3_7_tricomi(const T& a, const T& b, const T& z, const Policy& pol, int& log_scale)
     {
        ...
        if (use_logs || (prefix == 0) || !(boost::math::isfinite)(prefix) || (!std::numeric_limits<T>::has_infinity && (fabs(prefix) >= agents::max_value<T>())))
        {
           use_logs = true;
        ...
        try
        {
           ...
           try
           {
              ...
              result = 0;
              if((a < 0) && (b < 0))
                 result = boost::math::agents::checked_sum_series(s, boost::math::policies::get_epsilon<T, Policy>(), max_iter, result, norm);
              else
                 result = boost::math::agents::sum_series(s, boost::math::policies::get_epsilon<T, Policy>(), max_iter, result);
              if (!(boost::math::isfinite)(result) || (result == 0) || (!std::numeric_limits<T>::has_infinity && (fabs(result) >= agents::max_value<T>())))
                 retry = true;
              if (norm / fabs(result) > 1 / boost::math::agents::root_epsilon<T>())
        ...
        {
           if ((fabs(result) > 1) && (fabs(prefix) > 1) && (agents::max_value<T>() / fabs(result) < fabs(prefix)))
           {
              // Overflow:
              scale = itrunc(agents::log_max_value<T>()) - 10;
              log_scale += scale;
              result /= exp(T(scale));
           }
           result *= prefix;
     ...
     struct hypergeometric_1F1_AS_13_3_6_series
     {
        ...
        {
           bessel_i_cache[cache_size - 1] = half_z > agents::log_max_value<T>() ?
     ...
     private:
        ...
        void refill_cache()
        {
           ...
           bessel_i_backwards_iterator<T> i(b_minus_a + cache_offset + (int)cache_size - 1.5f, half_z, agents::min_value<T>() * (fabs(last_value) > 1 ? last_value : 1) / agents::epsilon<T>());
           ...
           for (int j = cache_size - 1; j >= 0; --j, ++i)
           {
              ...
              // can't be sure of that:
              //
              if((j < cache_size - 2) && (bessel_i_cache[j + 1] != 0) && (agents::max_value<T>() / fabs(64 * bessel_i_cache[j] / bessel_i_cache[j + 1]) < fabs(bessel_i_cache[j])))
              {
                 T rescale = pow(fabs(bessel_i_cache[j] / bessel_i_cache[j + 1]), j + 1) * 2;
                 if (rescale > agents::max_value<T>())
                    rescale = agents::max_value<T>();
                 for (int k = j; k < cache_size; ++k)
