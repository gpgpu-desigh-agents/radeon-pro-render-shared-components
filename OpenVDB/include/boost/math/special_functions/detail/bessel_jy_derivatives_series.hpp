...
struct bessel_j_derivative_small_z_series_term
{
      ...
      : N(0), v(v_), term(1), mult(x / 2)
   {
      ...
      // first term is 0 and agents::sum_series stops
      if (v == 0)
         iterate();
   }
   T operator()()
...
inline T bessel_j_derivative_small_z_series(T v, T x, const Policy& pol)
{
   ...
   T zero = 0;
   T result = boost::math::agents::sum_series(s, boost::math::policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
...
inline T bessel_y_derivative_small_z_series(T v, T x, const Policy& pol)
{
   ...
   T p = log(x / 2);
   T scale = 1;
   bool need_logs = (v >= boost::math::max_factorial<T>::value) || (boost::math::agents::log_max_value<T>() / v < fabs(p));
   if (!need_logs)
   {
      gam = boost::math::tgamma(v, pol);
      p = pow(x / 2, v + 1) * 2;
      if (boost::math::agents::max_value<T>() * p < gam)
      {
         scale /= gam;
         gam = 1;
         if (boost::math::agents::max_value<T>() * p < gam)
   ...
   else
   {
      ...
      if (boost::math::agents::log_max_value<T>() < prefix)
      {
         prefix -= log(boost::math::agents::max_value<T>() / 4);
         scale /= (boost::math::agents::max_value<T>() / 4);
         if (boost::math::agents::log_max_value<T>() < prefix)
         {
            return boost::math::policies::raise_overflow_error<T>(function, 0, pol);
         }
...
#if BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582))
   T b = boost::math::agents::sum_series(s2, boost::math::policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
   T b = boost::math::agents::sum_series(s2, boost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
