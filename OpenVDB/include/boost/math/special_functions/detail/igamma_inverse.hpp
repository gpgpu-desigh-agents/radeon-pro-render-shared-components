...
T find_inverse_s(T p, T q)
{
   ...
   T s = t - agents::evaluate_polynomial(a, t) / agents::evaluate_polynomial(b, t);
   if(p < 0.5)
      s = -s;
   return s;
}
...
struct gamma_p_inverse_func
{
   ...
   boost::math::tuple<T, T, T> operator()(const T& x)const
   {
      ...
      T div = (a - x - 1) / x;
      f2 = f1;
      if((fabs(div) > 1) && (agents::max_value<T>() / fabs(div) < f2))
      {
         // overflow:
         f2 = -agents::max_value<T>() / 2;
      }
      else
      {
         f2 *= div;
...
T gamma_p_inv_imp(T a, T p, const Policy& pol)
{
   ...
   if((policies::digits<T, Policy>() <= 36) && has_10_digits)
      return guess;
   T lower = agents::min_value<T>();
   if(guess <= lower)
      guess = agents::min_value<T>();
   ...
   if((a < 0.125) && (fabs(gamma_p_derivative(a, guess, pol)) > 1 / sqrt(agents::epsilon<T>())))
      digits = policies::digits<T, Policy>() - 2;
   //
   // Go ahead and iterate:
   //
   boost::uintmax_t max_iter = policies::get_max_root_iterations<Policy>();
   guess = agents::halley_iterate(
