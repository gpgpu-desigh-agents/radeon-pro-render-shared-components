...
inline T bessel_y_small_z_series(T v, T x, T* pscale, const Policy& pol)
{
   ...
   T p = log(x / 2);
   T scale = 1;
   bool need_logs = (v >= max_factorial<T>::value) || (agents::log_max_value<T>() / v < fabs(p));
   if(!need_logs)
   {
      gam = boost::math::tgamma(v, pol);
      p = pow(x / 2, v);
      if(agents::max_value<T>() * p < gam)
      {
         scale /= gam;
         gam = 1;
         if(agents::max_value<T>() * p < gam)
   ...
   else
   {
      ...
      if(agents::log_max_value<T>() < prefix)
      {
         prefix -= log(agents::max_value<T>() / 4);
         scale /= (agents::max_value<T>() / 4);
         if(agents::log_max_value<T>() < prefix)
         {
            return -policies::raise_overflow_error<T>(function, 0, pol);
         }
...
T bessel_yn_small_z(int n, T z, T* scale, const Policy& pol)
{
   ...
   else
   {
      ...
      if(p * agents::max_value<T>() < result)
      {
         T div = agents::max_value<T>() / 8;
         result /= div;
         *scale /= div;
         if(p * agents::max_value<T>() < result)
         {
