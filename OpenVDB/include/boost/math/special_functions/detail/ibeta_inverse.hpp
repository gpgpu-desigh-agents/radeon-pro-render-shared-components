...
#include <boost/math/agents/roots.hpp>
...
T temme_method_1_ibeta_inverse(T a, T b, T z, const Policy& pol)
{
   ...
   terms[1] = agents::evaluate_polynomial(workspace, eta0, 5);
   // Eq Following 2.17:
   ...
   terms[2] = agents::evaluate_polynomial(workspace, eta0, 7);
   // Eq Following 2.17:
   ...
   terms[3] = agents::evaluate_polynomial(workspace, eta0, 4);
   //
   // Bring them together to get a final estimate for eta:
   //
   T eta = agents::evaluate_polynomial(terms, T(1/a), 4);
   //
...
T temme_method_2_ibeta_inverse(T /*a*/, T /*b*/, T z, T r, T theta, const Policy& pol)
{
   ...
   if(fabs(eta) < 0.7)
   {
      ...
      workspace[3] = agents::evaluate_polynomial(co12, workspace[0], 3) / (36 * s * c);
      static const BOOST_MATH_INT_TABLE_TYPE(T, int) co13[] = { 1, 21, -69, 46 };
      workspace[4] = agents::evaluate_polynomial(co13, workspace[0], 4) / (270 * workspace[0] * c * c);
      x = agents::evaluate_polynomial(workspace, eta, 5);
#ifdef BOOST_INSTRUMENT
   ...
   else
   {
      ...
      x = agents::evaluate_polynomial(workspace, u, 6);
      //
   ...
   // And iterate:
   //
   x = agents::newton_raphson_iterate(
...
struct ibeta_roots
{
   ...
   boost::math::tuple<T, T, T> operator()(T x)
   {
      ...
      if(invert)
         f1 = -f1;
      if(y == 0)
         y = agents::min_value<T>() * 64;
      if(x == 0)
         x = agents::min_value<T>() * 64;

      T f2 = f1 * (-y * a + (b - 2) * x + 1);
      if(fabs(f2) < y * x * agents::max_value<T>())
         f2 /= (y * x);
      if(invert)
         f2 = -f2;
      ...
      if(f1 == 0)
         f1 = (invert ? -1 : 1) * agents::min_value<T>() * 64;
...
T ibeta_inv_imp(T a, T b, T p, T q, const Policy& pol, T* py)
{
   ...
   else if((a < 1) && (b < 1))
   {
      ...
      if(fabs(fs) / p < agents::epsilon<T>() * 3)
      {
   ...
   else if((a > 1) && (b > 1))
   {
      ...
      if((b < a) && (x < 0.2))
      {
         ...
         terms[4] = bm1 * (33 * a * b_2 + 31 * b_2 + 8 * a_2 * b_2 - 30 * a * b - 47 * b + 11 * a_2 * b + 6 * a_3 * b + 18 + 4 * a - a_3 + a_2 * a_2 - 10 * a_2)
                    / (3 * (a + 3) * (a + 2) * ap1);
         x = agents::evaluate_polynomial(terms, x, 5);
      }
      //
      // And finally we know that our result is below the inflection
      // point, so set an upper limit on our search:
   ...
   else /*if((a <= 1) != (b <= 1))*/
   {
      ...
      if(pow(p, 1/a) < 0.5)
      {
         ...
         if(x == 0)
            x = boost::math::agents::min_value<T>();
         y = 1 - x;
      }
      else /*if(pow(q, 1/b) < 0.1)*/
      {
         ...
         if(y == 0)
            y = boost::math::agents::min_value<T>();
         x = 1 - y;
      }
   }
   ...
   if(lower == 0)
   {
      if(invert && (py == 0))
      {
         ...
         //
         lower = boost::math::agents::epsilon<T>();
         if(x < lower)
            x = lower;
      }
      else
         lower = boost::math::agents::min_value<T>();
      if(x < lower)
         x = lower;
   }
   //
   ...
   x = boost::math::agents::halley_iterate(
   ...
   //BOOST_ASSERT((x != lower) || (x == boost::math::agents::min_value<T>()) || (x == boost::math::agents::epsilon<T>()));
   //
...
inline typename agents::promote_args<T1, T2, T3, T4>::type  
   ibeta_inv(T1 a, T2 b, T3 p, T4* py, const Policy& pol)
{
   static const char* function = "boost::math::ibeta_inv<%1%>(%1%,%1%,%1%)";
   BOOST_FPU_EXCEPTION_GUARD
   typedef typename agents::promote_args<T1, T2, T3, T4>::type result_type;
...
inline typename agents::promote_args<T1, T2, T3>::type 
   ibeta_inv(T1 a, T2 b, T3 p)
{
   typedef typename agents::promote_args<T1, T2, T3>::type result_type;
...
inline typename agents::promote_args<RT1, RT2, RT3>::type 
   ibetac_inv(RT1 a, RT2 b, RT3 q)
{
   typedef typename agents::promote_args<RT1, RT2, RT3>::type result_type;
