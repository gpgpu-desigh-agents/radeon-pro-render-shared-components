...
T bessel_i1_imp(const T& x, const mpl::int_<24>&)
{
   BOOST_MATH_STD_USING
      if(x < 7.75)
      {
         ...
         T a = x * x / 4;
         T Q[3] = { 1, 0.5f, boost::math::agents::evaluate_polynomial(P, a) };
         return x * boost::math::agents::evaluate_polynomial(Q, a) / 2;
      }
      else
      {
         ...
         T ex = exp(x / 2);
         T result = ex * boost::math::agents::evaluate_polynomial(P, T(1 / x)) / sqrt(x);
         result *= ex;
         return result;
      }
}
...
T bessel_i1_imp(const T& x, const mpl::int_<53>&)
{
   ...
   else if(x < 500)
   {
      ...
      return exp(x) * boost::math::agents::evaluate_polynomial(P, T(1 / x)) / sqrt(x);
   }
   else
   {
