...
T bessel_k0_imp(const T& x, const mpl::int_<24>&)
{
   ...
   if(x <= 1)
   {
      ...
      T a = x * x / 4;
      a = (agents::evaluate_rational(P, Q, a) + Y) * a + 1;
      ...
      return agents::evaluate_polynomial(P2, T(x * x)) - log(x) * a;
   }
   else
   {
      ...
      if(x < agents::log_max_value<T>())
         return ((agents::evaluate_rational(P, Q, T(1 / x)) + 1) * exp(-x) / sqrt(x));
      else
      {
         T ex = exp(-x / 2);
         return ((agents::evaluate_rational(P, Q, T(1 / x)) + 1) * ex / sqrt(x)) * ex;
      }
   }
}
...
T bessel_k0_imp(const T& x, const mpl::int_<53>&)
{
   ...
   if(x <= 1)
   {
      ...
      T a = x * x / 4;
      a = (agents::evaluate_polynomial(P, a) / agents::evaluate_polynomial(Q, a) + Y) * a + 1;
...
T bessel_k0_imp(const T& x, const mpl::int_<64>&)
{
   BOOST_MATH_STD_USING
      if(x <= 1)
      {
         ...
         return agents::evaluate_rational(P2, Q2, T(x * x)) - log(x) * a;
      }
      else
      {
