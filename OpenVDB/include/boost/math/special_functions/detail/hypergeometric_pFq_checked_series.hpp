...
#include <boost/math/special_functions/detail/hypergeometric_series.hpp>
  ...
  namespace boost { namespace math { namespace detail {
     ...
     std::pair<Real, Real> hypergeometric_pFq_checked_series_impl(const Seq& aj, const Seq& bj, const Real& z, const Policy& pol, const Terminal& termination, int& log_scale)
     {
        ...
        boost::uintmax_t k = 0;
        Real upper_limit(sqrt(boost::math::agents::max_value<Real>())), diff;
        Real lower_limit(1 / upper_limit);
        int log_scaling_factor = itrunc(boost::math::agents::log_max_value<Real>()) - 2;
        ...
        for (unsigned n = 0; n < N_crossovers; ++n)
        {
           if (k < crossover_locations[n])
           {
              ...
              //
              loop_error_scale *= agents::epsilon<Real>();
              //
              ...
              if (term <= agents::log_min_value<Real>())
              {
                 // rescale if we can:
                 int scale = itrunc(floor(term - agents::log_min_value<Real>()) - 2);
                 term -= scale;
                 loop_scale += scale;
              }
               if (term > 10)
               ...
               do
               {
                  ...
                  if (!trivial_small_series_check && !terms_are_growing)
                  {
                     ...
                     if (loop_scale > local_scaling)
                     {
                        ...
                        if (rescale < agents::log_min_value<Real>())
