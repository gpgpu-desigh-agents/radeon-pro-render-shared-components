...
T erf_inv_imp(const T& p, const T& q, const Policy&, const boost::mpl::int_<64>*)
{
   ...
   if(p <= 0.5)
   {
      ...
      T g = p * (p + 10);
      T r = agents::evaluate_polynomial(P, p) / agents::evaluate_polynomial(Q, p);
      result = g * Y + g * r;
   }
   else if(q >= 0.25)
   {
      ...
      T g = sqrt(-2 * log(q));
      T xs = q - 0.25f;
      T r = agents::evaluate_polynomial(P, xs) / agents::evaluate_polynomial(Q, xs);
      result = g / (Y + r);
   }
   else
   {
...
T erf_inv_imp(const T& p, const T& q, const Policy& pol, const boost::mpl::int_<0>*)
{
   ...
   if(policies::digits<T, Policy>() > 64)
   {
      ...
      if(p <= 0.5)
      {
         result = agents::halley_iterate(detail::erf_roots<typename remove_cv<T>::type, Policy>(p, 1), guess, static_cast<T>(0), agents::max_value<T>(), (policies::digits<T, Policy>() * 2) / 3, max_iter);
      }
      else
      {
         result = agents::halley_iterate(detail::erf_roots<typename remove_cv<T>::type, Policy>(q, -1), guess, static_cast<T>(0), agents::max_value<T>(), (policies::digits<T, Policy>() * 2) / 3, max_iter);
      }
   ...
   struct init
   {
      ...
      static void do_init()
      {
         ...
         if(std::numeric_limits<T>::digits)
         {
            ...
            // some agents such as valgrind silently change the precision
...
template <class T, class Policy>
typename agents::promote_args<T>::type erfc_inv(T z, const Policy& pol)
{
...
template <class T, class Policy>
typename agents::promote_args<T>::type erf_inv(T z, const Policy& pol)
{
   typedef typename agents::promote_args<T>::type result_type;
...
template <class T>
inline typename agents::promote_args<T>::type erfc_inv(T z)
{
   return erfc_inv(z, policies::policy<>());
}

template <class T>
inline typename agents::promote_args<T>::type erf_inv(T z)
{
