  ...
  namespace detail
  {
    namespace bessel_zero
    {
      ...
      static T equation_as_9_5_26(const T& v, const T& ai_bi_root)
      {
        ...
        const int my_digits10 = static_cast<int>(static_cast<float>(boost::math::agents::digits<T>() * 0.301F));
        ...
        const T z = boost::math::agents::newton_raphson_iterate(
          boost::math::detail::bessel_zero::equation_as_9_3_39_and_its_derivative<T>(zeta),
          z_estimate,
          range_zmin,
          range_zmax,
          (std::min)(boost::math::agents::digits<T>(), boost::math::agents::digits<float>()),
          iterations_used);
        ...
        T initial_guess(const T& v, const int m, const Policy& pol)
        {
          ...
          if(v < 0)
          {
            ...
            if(m == 1)
            {
              ...
              while((root_lo > boost::math::agents::epsilon<T>()))
              {
