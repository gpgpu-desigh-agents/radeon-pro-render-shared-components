...
int temme_ik(T v, T x, T* K, T* K1, const Policy& pol)
{
    ...
    b = exp(v * a);
    sigma = -a * v;
    c = abs(v) < agents::epsilon<T>() ?
       T(1) : T(boost::math::sin_pi(v) / (v * pi<T>()));
    d = abs(sigma) < agents::epsilon<T>() ?
        T(1) : T(sinh(sigma) / sigma);
    gamma1 = abs(v) < agents::epsilon<T>() ?
...
int CF1_ik(T v, T x, T* fv, const Policy& pol)
{
    ...
    tolerance = 2 * agents::epsilon<T>();
    BOOST_MATH_INSTRUMENT_VARIABLE(tolerance);
    tiny = sqrt(agents::min_value<T>());
...
int bessel_ik(T v, T x, T* I, T* K, int kind, const Policy& pol)
{
    ...
    {
        T fact = 2 * (u + k) / x;
        if((agents::max_value<T>() - fabs(prev)) / fact < fabs(current))
        {
           prev /= current;
           scale /= current;
    ...
    if(kind & need_i)
    {
       ...
       lim *= lim;
       lim /= 24;
       if((lim < agents::epsilon<T>() * 10) && (x > 100))
       {
    ...
    if (reflect)
    {
        ...
        if(fact == 0)
           *I = Iv;
        else if(agents::max_value<T>() * scale < fact)
    ...
    if(agents::max_value<T>() * scale < Kv)
