    ...
    namespace math
    {
       namespace detail{
        ...
        inline T    asinh_imp(const T x, const Policy& pol)
        {
            ...
            if        (x >= agents::forth_root_epsilon<T>())
            {
               if        (x > 1 / agents::root_epsilon<T>())
                {
            ...
            else if    (x <= -agents::forth_root_epsilon<T>())
            {
                return(-asinh(-x, pol));
            }
            else
            {
                ...
                if    (abs(x) >= agents::root_epsilon<T>())
                {
                    T    x3 = x*x*x;
        ...
        template<typename T>
        inline typename agents::promote_args<T>::type asinh(T x)
        {
           return boost::math::asinh(x, policies::policy<>());
        }
        template<typename T, typename Policy>
        inline typename agents::promote_args<T>::type asinh(T x, const Policy&)
        {
