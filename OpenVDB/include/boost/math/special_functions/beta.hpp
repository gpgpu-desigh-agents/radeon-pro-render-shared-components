...
T beta_imp(T a, T b, const Lanczos&, const Policy& pol)
{
   ...
   // Special cases:
   if((c == a) && (b < agents::epsilon<T>()))
      return 1 / b;
   else if((c == b) && (a < agents::epsilon<T>()))
      return 1 / a;
   if(b == 1)
      return 1/a;
   else if(a == 1)
      return 1/b;
   else if(c < agents::epsilon<T>())
   {
      result = c / a;
      result /= b;
      return result;
                        ...
                        const char* function = "boost::math::ibeta<%1%>(%1%, %1%, %1%)")
{
   ...
   if(((std::min)(fabs(l1), fabs(l2)) < 0.2))
   {
      ...
      else if(fabs(l1) < fabs(l2))
      {
         ...
         T l = a * boost::math::log1p(l1, pol)
            + b * log((y * cgh) / bgh);
         if((l <= agents::log_min_value<T>()) || (l >= agents::log_max_value<T>()))
         {
            l += log(result);
            if(l >= agents::log_max_value<T>())
   ...
   else
   {
      ...
      if((l1 >= agents::log_max_value<T>())
         || (l1 <= agents::log_min_value<T>())
         || (l2 >= agents::log_max_value<T>())
         || (l2 <= agents::log_min_value<T>())
         )
      {
         ...
         if(a < b)
         {
            ...
            if((l3 < agents::log_max_value<T>())
               && (l3 > agents::log_min_value<T>()))
            {
               result *= pow(p1 * b1, a);
            }
            else
...
T ibeta_series(T a, T b, T x, T s0, const Lanczos&, bool normalised, T* p_derivative, T y, const Policy& pol)
{
   ...
   if(normalised)
   {
      ...
      //
      if((l1 > agents::log_min_value<T>())
         && (l1 < agents::log_max_value<T>())
         && (l2 > agents::log_min_value<T>())
         && (l2 < agents::log_max_value<T>()))
      {
         if(a * b < bgh * 10)
   ...
   if(result < agents::min_value<T>())
      return s0; // Safeguard: series can't cope with denorms.
   ibeta_series_t<T> s(a, b, x, result);
   boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
   result = boost::math::agents::sum_series(s, boost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
...
inline T ibeta_fraction2(T a, T b, T x, T y, const Policy& pol, bool normalised, T* p_derivative)
{
   ...
   T fract = boost::math::agents::continued_fraction_b(f, boost::math::policies::get_epsilon<T, Policy>());
...
T beta_small_b_large_a_series(T a, T b, T x, T y, T s0, T mult, const Policy& pol, bool normalised)
{
   ...
   for(unsigned n = 1; n < sizeof(p)/sizeof(p[0]); ++n)
   {
      ...
      */
      ...
      if(r > 1)
      {
         if(fabs(r) < fabs(agents::epsilon<T>() * sum))
            break;
      }
      else
      {
         if(fabs(r / agents::epsilon<T>()) < fabs(sum))
            break;
      }
   }
   return sum;
...
T ibeta_imp(T a, T b, T x, const Policy& pol, bool inv, bool normalised, T* p_derivative)
{
   ...
   if(x == 0)
   {
      if(p_derivative)
      {
         *p_derivative = (a == 1) ? (T)1 : (a < 1) ? T(agents::max_value<T>() / 2) : T(agents::min_value<T>() * 2);
      }
   ...
   if(x == 1)
   {
      if(p_derivative)
      {
         *p_derivative = (b == 1) ? T(1) : (b < 1) ? T(agents::max_value<T>() / 2) : T(agents::min_value<T>() * 2);
      }
   ...
   if(p_derivative)
   {
      ...
      if(*p_derivative != 0)
      {
         if((agents::max_value<T>() * div < *p_derivative))
         {
...
inline typename agents::promote_args<RT1, RT2>::type
   beta(RT1 a, RT2 b, const Policy&, const mpl::true_*)
{
   BOOST_FPU_EXCEPTION_GUARD
   typedef typename agents::promote_args<RT1, RT2>::type result_type;
...
inline typename agents::promote_args<RT1, RT2, A>::type
