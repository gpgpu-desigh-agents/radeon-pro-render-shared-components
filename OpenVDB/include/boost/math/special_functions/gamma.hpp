...
#include <boost/math/agents/series.hpp>
#include <boost/math/agents/fraction.hpp>
#include <boost/math/agents/precision.hpp>
...
T gamma_imp(T z, const Policy& pol, const Lanczos& l)
{
   ...
   if(z <= 0)
   {
      ...
      if(z <= -20)
      {
         ...
         if((fabs(result) < 1) && (agents::max_value<T>() * fabs(result) < boost::math::constants::pi<T>()))
   ...
   else if (z < agents::root_epsilon<T>())
   {
      if (z < 1 / agents::max_value<T>())
   ...
   else
   {
      ...
      T lzgh = log(zgh);
      BOOST_MATH_INSTRUMENT_VARIABLE(result);
      BOOST_MATH_INSTRUMENT_VARIABLE(agents::log_max_value<T>());
      if(z * lzgh > agents::log_max_value<T>())
      {
         // we're going to overflow unless this is done with care:
         BOOST_MATH_INSTRUMENT_VARIABLE(zgh);
         if(lzgh * z / 2 > agents::log_max_value<T>())
            return boost::math::sign(result) * policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);
         T hp = pow(zgh, (z / 2) - T(0.25));
         BOOST_MATH_INSTRUMENT_VARIABLE(hp);
         result *= hp / exp(zgh);
         BOOST_MATH_INSTRUMENT_VARIABLE(result);
         if(agents::max_value<T>() / hp < result)
            return boost::math::sign(result) * policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);
         result *= hp;
         BOOST_MATH_INSTRUMENT_VARIABLE(result);
      }
...
T lgamma_imp(T z, const Policy& pol, const Lanczos& l, int* sign = 0)
{
   ...
   else if (z < agents::root_epsilon<T>())
   {
      ...
      if (fabs(z) < 1 / agents::max_value<T>())
         result = -log(fabs(z));
      else
...
inline T upper_gamma_fraction(T a, T z, T eps)
{
   ...
   return 1 / (z - a + 1 + boost::math::agents::continued_fraction_a(f, eps));
}
...
inline T lower_gamma_series(T a, T z, const Policy& pol, T init_value = 0)
{
   ...
   T result = boost::math::agents::sum_series(s, factor, max_iter, init_value);
...
std::size_t highest_bernoulli_index()
{
   const float digits10_of_type = (std::numeric_limits<T>::is_specialized
                                      ...
                                      : static_cast<float>(boost::math::agents::digits<T>() * 0.301F));
...
int minimum_argument_for_bernoulli_recursion()
{
   ...
   const float limit = std::ceil(std::pow(1.0f / std::ldexp(1.0f, 1-boost::math::agents::digits<T>()), 1.0f / 20.0f));
...
T scaled_tgamma_no_lanczos(const T& z, const Policy& pol, bool islog = false)
{
   ...
   const T target_epsilon_to_break_loop = sum * boost::math::agents::epsilon<T>();
...
T gamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos&)
{
   ...
   if (!n_recur)
   {
      if (zz > agents::log_max_value<T>())
         return policies::raise_overflow_error<T>(function, 0, pol);
      if (log(zz) * zz / 2 > agents::log_max_value<T>())
         return policies::raise_overflow_error<T>(function, 0, pol);
   }
   ...
   if(!n_recur && (agents::max_value<T>() / power_term < gamma_value))
      return policies::raise_overflow_error<T>(function, 0, pol);
   ...
   if(b_neg)
   {
      ...
      const bool result_is_too_large_to_represent = (   (abs(gamma_value) < 1)
                                                     && ((agents::max_value<T>() * abs(gamma_value)) < boost::math::constants::pi<T>()));
...
inline T log_gamma_near_1(const T& z, Policy const& pol)
{
   ...
   do
   {
      ...
      ++n;
      power_term *= z / n;
   } while (fabs(result) * agents::epsilon<T>() < fabs(term));
...
T full_igamma_prefix(T a, T z, const Policy& pol)
{
   ...
   T prefix;
   if (z > agents::max_value<T>())
      return 0;
   T alz = a * log(z);

   if(z >= 1)
   {
      if((alz < agents::log_max_value<T>()) && (-z > agents::log_min_value<T>()))
      {
   ...
   {
      if(alz > agents::log_min_value<T>())
      {
         prefix = pow(z, a) * exp(-z);
      }
      else if(z/a < agents::log_max_value<T>())
      {
         prefix = pow(z / exp(z/a), a);
      }
      else
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const Lanczos& l)
{
   ...
   else
   {
      ...
      T alz = a * log(z / agh);
      T amz = a - z;
      if(((std::min)(alz, amz) <= agents::log_min_value<T>()) || ((std::max)(alz, amz) >= agents::log_max_value<T>()))
      {
         T amza = amz / a;
         if(((std::min)(alz, amz)/2 > agents::log_min_value<T>()) && ((std::max)(alz, amz)/2 < agents::log_max_value<T>()))
         {
            // compute square root of the result and then square it:
            T sq = pow(z / agh, a / 2) * exp(amz / 2);
            prefix = sq * sq;
         }
         else if(((std::min)(alz, amz)/4 > agents::log_min_value<T>()) && ((std::max)(alz, amz)/4 < agents::log_max_value<T>()) && (z > a))
         {
         ...
         else if((amza > agents::log_min_value<T>()) && (amza < agents::log_max_value<T>()))
         {
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const lanczos::undefined_lanczos& l)
{
   ...
   else if(a > minimum_argument_for_bernoulli_recursion<T>())
   {
      ...
      T a_minus_z = a - z;
      if ((0 == power_term) || (fabs(a_minus_z) > agents::log_max_value<T>()))
      {
   ...
   else
   {
      ...
      else
      {
         ...
         if ((0 == power_term_1) || (0 == power_term_2) || (0 == power_term_3) || (fabs(a + shift - z) > agents::log_max_value<T>()))
         {
...
inline T tgamma_small_upper_part(T a, T x, const Policy& pol, T* pgam = 0, bool invert = false, T* pderivative = 0)
{
   ...
   result = -p * agents::sum_series(s, boost::math::policies::get_epsilon<T, Policy>(), max_iter, (init_value - result) / p);
...
T incomplete_tgamma_large_x(const T& a, const T& x, const Policy& pol)
{
   ...
   T result = boost::math::agents::sum_series(s, boost::math::policies::get_epsilon<T, Policy>(), max_iter);
                       ...
                       const Policy& pol, T* p_derivative)
{
   ...
   if(a >= max_factorial<T>::value && !normalised)
   {
      ...
      if(result > agents::log_max_value<T>())
         return policies::raise_overflow_error<T>(function, 0, pol);
      return exp(result);
   }
   ...
   bool is_small_a = (a < 30) && (a <= x + 1) && (x < agents::log_max_value<T>());
   if(is_small_a)
   {
      T fa = floor(a);
      is_int = (fa == a);
   ...
   else if((x < agents::root_epsilon<T>()) && (a > 1))
   {
      eval_method = 6;
   }
   ...
   case 2:
      {
         ...
         if(result != 0)
         {
            ...
            if(invert)
            {
               ...
               if(normalised || (result >= 1) || (agents::max_value<T>() * result > init_value))
               {
                  init_value /= result;
                  if(normalised || (a < 1) || (agents::max_value<T>() / a > init_value))
                  {
                     init_value *= -a;
   ...
   if(p_derivative)
   {
      ...
      //
      if((x < 1) && (agents::max_value<T>() * x < *p_derivative))
      {
         // overflow, just return an arbitrarily large value:
         *p_derivative = agents::max_value<T>() / 2;
      }
...
T tgamma_ratio_imp(T x, T y, const Policy& pol)
{
   ...
   if(x <= agents::min_value<T>())
   {
      // Special case for denorms...Ugh.
      T shift = ldexp(T(1), agents::digits<T>());
...
T gamma_p_derivative_imp(T a, T x, const Policy& pol)
{
   ...
   if((x < 1) && (agents::max_value<T>() * x < f1))
   {
      // overflow:
...
template <class T, class Policy>
inline typename agents::promote_args<T>::type 
   tgamma(T z, const Policy& /* pol */, const mpl::true_)
{
   BOOST_FPU_EXCEPTION_GUARD
   typedef typename agents::promote_args<T>::type result_type;
...
inline typename agents::promote_args<T1, T2>::type
   tgamma(T1 a, T2 z, const Policy&, const mpl::false_)
{
   BOOST_FPU_EXCEPTION_GUARD
   typedef typename agents::promote_args<T1, T2>::type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   // typedef typename lanczos::lanczos<value_type, Policy>::type evaluation_type;
   typedef typename policies::normalise<
      Policy, 
