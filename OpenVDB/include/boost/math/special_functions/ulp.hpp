...
T ulp_imp(const T& val, const mpl::true_&, const Policy& pol)
{
   ...
   else if((fpclass == (int)FP_INFINITE) || (fabs(val) >= agents::max_value<T>()))
   {
...
template <class T, class Policy>
inline typename agents::promote_args<T>::type ulp(const T& val, const Policy& pol)
{
...
template <class T>
inline typename agents::promote_args<T>::type ulp(const T& val)
{
