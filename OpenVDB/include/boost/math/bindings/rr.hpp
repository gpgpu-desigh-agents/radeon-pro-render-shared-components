...
namespace ntl{
   ...
   inline RR asin(RR z)
   {
      double r;
      conv(r, z.value());
      return boost::math::agents::halley_iterate(
         asin_root(z), 
         RR(std::asin(r)), 
   ...
   {
      return agents::real_cast<int>(round(x, pol));
   }

   template <class Policy>
   inline long lround(RR const& x, const Policy& pol)
   {
      return agents::real_cast<long>(round(x, pol));
   }

   template <class Policy>
   inline long long llround(RR const& x, const Policy& pol)
   {
      return agents::real_cast<long long>(round(x, pol));
   }

   template <class Policy>
   inline int itrunc(RR const& x, const Policy& pol)
   {
      return agents::real_cast<int>(trunc(x, pol));
   }

   template <class Policy>
   inline long ltrunc(RR const& x, const Policy& pol)
   {
      return agents::real_cast<long>(trunc(x, pol));
   }

   template <class Policy>
   inline long long lltrunc(RR const& x, const Policy& pol)
   {
      return agents::real_cast<long long>(trunc(x, pol));
   }
