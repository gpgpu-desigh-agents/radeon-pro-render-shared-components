...
// see boost/math/agents/ntl.hpp  for examples like
...
#ifdef __SUNPRO_CC
   static const T m = boost::math::agents::max_value<T>();
   static const T val = log(m);
#else
   static const T val = log(boost::math::agents::max_value<T>());
#endif
   return val;
}
...
inline T log_min_value(const mpl::int_<0>& BOOST_MATH_APPEND_EXPLICIT_TEMPLATE_TYPE(T))
{
   BOOST_MATH_STD_USING
#ifdef __SUNPRO_CC
   static const T m = boost::math::agents::min_value<T>();
   static const T val = log(m);
#else
   static const T val = log(boost::math::agents::min_value<T>());
#endif
   return val;
}
...
{
   BOOST_MATH_STD_USING
   static const T r_eps = sqrt(agents::epsilon<T>());
   return r_eps;
}
...
{
   BOOST_MATH_STD_USING;
   static const T cbrt_eps = pow(agents::epsilon<T>(), T(1) / 3);
   return cbrt_eps;
}
...
{
   BOOST_MATH_STD_USING;
   return pow(agents::epsilon<T>(), T(1) / 3);
}
...
{
   BOOST_MATH_STD_USING
   static const T r_eps = sqrt(sqrt(agents::epsilon<T>()));
   return r_eps;
}
...
{
   BOOST_MATH_STD_USING
   return sqrt(sqrt(agents::epsilon<T>()));
}
