...
#  define BOOST_MATH_LOGGER_INCLUDE <boost/math/agents/iteration_logger.hpp>
...
public:
   eps_tolerance()
   {
      eps = 4 * agents::epsilon<T>();
   }
   eps_tolerance(unsigned bits)
   {
      BOOST_MATH_STD_USING
      eps = (std::max)(T(ldexp(1.0F, 1-bits)), T(4 * agents::epsilon<T>()));
   }
...
inline T safe_div(T num, T denom, T r)
{
   ...
   if(fabs(denom) < 1)
   {
      if(fabs(denom * agents::max_value<T>()) <= fabs(num))
         return r;
   }
   return num / denom;
}
                           ...
                           unsigned count)
{
   ...
   //
   T B = safe_div(T(fb - fa), T(b - a), agents::max_value<T>());
   T A = safe_div(T(fd - fb), T(d - b), agents::max_value<T>());
...
std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, boost::uintmax_t& max_iter, const Policy& pol)
{
   ...
   static const char* function = "boost::math::agents::toms748_solve<%1%>";
   ...
   while(count && (fa != 0) && !tol(a, b))
   {
      ...
      //
      T min_diff = agents::min_value<T>() * 32;
...
{
   BOOST_MATH_STD_USING
   static const char* function = "boost::math::agents::bracket_and_solve_root<%1%>";
   //
