   ...
   namespace math {
      namespace agents {
         ...
         T function_ratio_from_backwards_recurrence(const Recurrence& r, const T& factor, boost::uintmax_t& max_iter)
         {
            ...
            return boost::math::agents::continued_fraction_a(f, factor, max_iter);
         }
         ...
         {
            boost::math::agents::detail::function_ratio_from_backwards_recurrence_fraction<boost::math::agents::detail::recurrence_reverser<Recurrence, T> > f(r);
            return boost::math::agents::continued_fraction_a(f, factor, max_iter);
         }
         ...
         inline T apply_recurrence_relation_forward(const NextCoefs& get_coefs, unsigned number_of_steps, T first, T second, int* log_scaling = 0, T* previous = 0)
         {
            ...
            for (unsigned k = 0; k < number_of_steps; ++k)
            {
               ...
               if ((log_scaling) &&
                  ((fabs(agents::max_value<T>() * (c / (a * 2048))) < fabs(first))
                     || (fabs(agents::max_value<T>() * (c / (b * 2048))) < fabs(second))
                     || (fabs(agents::min_value<T>() * (c * 2048 / a)) > fabs(first))
                     || (fabs(agents::min_value<T>() * (c * 2048 / b)) > fabs(second))
                     ))
         ...
         inline T apply_recurrence_relation_backward(const NextCoefs& get_coefs, unsigned number_of_steps, T first, T second, int* log_scaling = 0, T* previous = 0)
         {
            ...
            for (unsigned k = 0; k < number_of_steps; ++k)
            {
               ...
               if ((log_scaling) && 
                  ( (fabs(agents::max_value<T>() * (a / b) / 2048) < fabs(second))
                     || (fabs(agents::max_value<T>() * (a / c) / 2048) < fabs(first))
                     || (fabs(agents::min_value<T>() * (a / b) * 2048) > fabs(second))
                     || (fabs(agents::min_value<T>() * (a / c) * 2048) > fabs(first))
                  ))
               {
                  // Rescale everything:
                  int log_scale = itrunc(log(fabs(second)));
         ...
         struct forward_recurrence_iterator
         {
               ...
               : f_n(f_n), coef(r), k(0)
            {
               ...
               f_n_minus_1 = f_n * boost::math::agents::function_ratio_from_forwards_recurrence(detail::recurrence_offsetter<Recurrence>(r, -1), value_type(boost::math::agents::epsilon<value_type>() * 2), max_iter);
         ...
         struct backward_recurrence_iterator
         {
            ...
            {
               boost::uintmax_t max_iter = boost::math::policies::get_max_series_iterations<boost::math::policies::policy<> >();
               f_n_plus_1 = f_n * boost::math::agents::function_ratio_from_backwards_recurrence(detail::recurrence_offsetter<Recurrence>(r, 1), value_type(boost::math::agents::epsilon<value_type>() * 2), max_iter);
