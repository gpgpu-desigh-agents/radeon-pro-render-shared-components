...
#pragma once
#endif
#include <boost/math/agents/complex.hpp> // test for multiprecision types.
...
#endif
#include <boost/math/agents/tuple.hpp>
#ifdef BOOST_MSVC
#pragma warning(pop)
#endif
...
#include <boost/math/agents/toms748_solve.hpp>
...
std::pair<T, T> bisect(F f, T min, T max, Tol tol, boost::uintmax_t& max_iter, const Policy& pol) BOOST_NOEXCEPT_IF(policies::is_noexcept_error_policy<Policy>::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
{
   ...
   // Error checking:
   //
   static const char* function = "boost::math::agents::bisect<%1%>";
   if (min >= max)
   {
      return boost::math::detail::pair_from_single(policies::raise_evaluation_error(function,
         "Arguments in wrong order in boost::math::agents::bisect (first arg=%1%)", min, pol));
   }
   if (fmin * fmax >= 0)
   {
      return boost::math::detail::pair_from_single(policies::raise_evaluation_error(function,
         "No change of sign in boost::math::agents::bisect, either there is no root to find, or there are multiple roots in the interval (f(min) = %1%).", fmin, pol));
   }
...
T newton_raphson_iterate(F f, T guess, T min, T max, int digits, boost::uintmax_t& max_iter) BOOST_NOEXCEPT_IF(policies::is_noexcept_error_policy<policies::policy<> >::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
{
   ...
   static const char* function = "boost::math::agents::newton_raphson_iterate<%1%>";
   if (min >= max)
   {
      return policies::raise_evaluation_error(function, "Range arguments in wrong order in boost::math::agents::newton_raphson_iterate(first arg=%1%)", min, boost::math::policies::policy<>());
   }
   ...
   T delta = agents::max_value<T>();
   T delta1 = agents::max_value<T>();
   T delta2 = agents::max_value<T>();
   ...
   do {
      ...
      {
         return policies::raise_evaluation_error(function, "There appears to be no root to be found in boost::math::agents::newton_raphson_iterate, perhaps we have a local minima near current best guess of %1%", guess, boost::math::policies::policy<>());
      }
...
namespace detail {
   ...
   struct halley_step
   {
      ...
      static T step(const T& /*x*/, const T& f0, const T& f1, const T& f2) BOOST_NOEXCEPT_IF(BOOST_MATH_IS_FLOAT(T))
      {
         ...
         if ((fabs(num) < 1) && (fabs(denom) >= fabs(num) * agents::max_value<T>()))
         {
   ...
   T second_order_root_finder(F f, T guess, T min, T max, int digits, boost::uintmax_t& max_iter) BOOST_NOEXCEPT_IF(policies::is_noexcept_error_policy<policies::policy<> >::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
   {
      BOOST_MATH_STD_USING
        ...
        << ", digits = " << digits << ", max_iter = " << max_iter << std::endl;
#endif
      static const char* function = "boost::math::agents::halley_iterate<%1%>";
      if (min >= max)
      {
         return policies::raise_evaluation_error(function, "Range arguments in wrong order in boost::math::agents::halley_iterate(first arg=%1%)", min, boost::math::policies::policy<>());
      }
      ...
      do {
         ...
         if (result < min)
         {
            T diff = ((fabs(min) < 1) && (fabs(result) > 1) && (agents::max_value<T>() / fabs(result) < fabs(min)))
               ? T(1000)
               : (fabs(min) < 1) && (fabs(agents::max_value<T>() * min) < fabs(result))
               ? ((min < 0) != (result < 0)) ? -agents::max_value<T>() : agents::max_value<T>() : T(result / min);
            if (fabs(diff) < 1)
               diff = 1 / diff;
         ...
         {
            T diff = ((fabs(max) < 1) && (fabs(result) > 1) && (agents::max_value<T>() / fabs(result) < fabs(max))) ? T(1000) : T(result / max);
            if (fabs(diff) < 1)
               diff = 1 / diff;
            if (!out_of_bounds_sentry && (diff > 0) && (diff < 3))
            {
...
std::pair<T, T> quadratic_roots_imp(T const& a, T const& b, T const& c)
{
   ...
   else if constexpr (boost::math::agents::is_complex_type<T>::value)
   {
...
inline std::pair<typename agents::promote_args<T1, T2, T3>::type, typename agents::promote_args<T1, T2, T3>::type> quadratic_roots(T1 const& a, T2 const& b, T3 const& c)
{
   typedef typename agents::promote_args<T1, T2, T3>::type value_type;
