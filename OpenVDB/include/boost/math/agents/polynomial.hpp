...
#include <boost/math/agents/detail/is_const_iterable.hpp>
...
public:
   ...
   polynomial(const polynomial<U>& p)
   {
      ...
      {
         m_data[i] = boost::math::agents::real_cast<T>(p[i]);
      }
   }
#ifdef BOOST_MATH_HAS_IS_CONST_ITERABLE
    template <class Range>
    explicit polynomial(const Range& r, typename boost::enable_if<boost::math::agents::detail::is_const_iterable<Range> >::type* = 0) 
   ...
   {
      return m_data.size() > 0 ? boost::math::agents::evaluate_polynomial(&m_data[0], z, m_data.size()) : T(0);
   }
...
//
#include <boost/math/agents/polynomial_gcd.hpp>
