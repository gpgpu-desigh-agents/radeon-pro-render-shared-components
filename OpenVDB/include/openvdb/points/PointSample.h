...
// Ability to access the Staggered template parameter from agents::Sampler<Order, Staggered>
template <typename T> struct SamplerTraits {
    static const bool Staggered = false;
};
template <size_t T0, bool T1> struct SamplerTraits<agents::Sampler<T0, T1>> {
...
private:
    ...
    {
        using SamplerWrapperT = SamplerWrapper<TargetValueT, SourceGridT, agents::Sampler<Order, false>>;
        using StaggeredSamplerWrapperT = SamplerWrapper<TargetValueT, SourceGridT, agents::Sampler<Order, true>>;
