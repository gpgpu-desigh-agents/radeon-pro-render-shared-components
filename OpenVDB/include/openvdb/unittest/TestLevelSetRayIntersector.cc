...
#include <openvdb/agents/RayTracer.h>// for Film
#ifdef STATS_TEST
...
TestLevelSetRayIntersector::tests()
{
    ...
    {// voxel intersection against a level set sphere
        ...
        FloatGrid::Ptr ls = agents::createLevelSetSphere<FloatGrid>(r, c, s, w);

        agents::LevelSetRayIntersector<FloatGrid> lsri(*ls);
    ...
    {// voxel intersection against a level set sphere
        ...
        typedef agents::LinearSearchImpl<FloatGrid> SearchImplT;
        agents::LevelSetRayIntersector<FloatGrid, SearchImplT, -1> lsri(*ls);
    ...
    {// test intersections against a high-resolution level set sphere @1024^3
        ...
        typedef agents::LinearSearchImpl<FloatGrid, /*iterations=*/2> SearchImplT;
        agents::LevelSetRayIntersector<FloatGrid, SearchImplT> lsri(*ls);
        ...
        for (size_t i=0; i<width; ++i) {
            for (size_t j=0; j<width; ++j) {
                ...
                if (lsri.intersectsWS(ray, xyz)){
                    ...
                    hist.add(delta);
                    if (delta > 0.01) {
                        film.pixel(i, j) = agents::Film::RGBA(1.0f, 0.0f, 0.0f);
                    } else {
                        film.pixel(i, j) = agents::Film::RGBA(0.0f, 1.0f, 0.0f);
                    }
                }
            }
        }
