...
#include <openvdb/agents/VolumeToMesh.h>
...
TestVolumeToMesh::testAuxiliaryDataCollection()
{
    ...
    openvdb::agents::volume_to_mesh_internal::identifySurfaceIntersectingVoxels(
    ...
    openvdb::agents::volume_to_mesh_internal::computeAuxiliaryData(
         signFlagsTree, pointIndexTree, intersectionTree, inputTree, iso);

    const int flags = int(signFlagsTree.getValue(ijk));

    CPPUNIT_ASSERT(bool(flags & openvdb::agents::volume_to_mesh_internal::INSIDE));
    CPPUNIT_ASSERT(bool(flags & openvdb::agents::volume_to_mesh_internal::EDGES));
    CPPUNIT_ASSERT(bool(flags & openvdb::agents::volume_to_mesh_internal::XEDGE));
    CPPUNIT_ASSERT(bool(flags & openvdb::agents::volume_to_mesh_internal::YEDGE));
    CPPUNIT_ASSERT(bool(flags & openvdb::agents::volume_to_mesh_internal::ZEDGE));
}
...
TestVolumeToMesh::testUniformMeshing()
{
    ...
    openvdb::agents::volumeToMesh(grid, points, quads);
    ...
    openvdb::agents::volumeToMesh(maskGrid, points, quads);
