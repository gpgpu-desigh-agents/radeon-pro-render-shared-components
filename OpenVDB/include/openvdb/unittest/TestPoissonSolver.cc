...
#include <openvdb/agents/Composite.h> // for csgDifference/Union/Intersection
#include <openvdb/agents/LevelSetSphere.h> // for agents::createLevelSetSphere()
#include <openvdb/agents/LevelSetUtil.h> // for agents::sdfToFogVolume()
#include <openvdb/agents/MeshToVolume.h> // for createLevelSetBox()
#include <openvdb/agents/Morphology.h> // for agents::erodeVoxels()
#include <openvdb/agents/PoissonSolver.h>
...
TestPoissonSolver::testLaplacian()
{
    ...
    for (int N = 8; N <= 20; N += 12) {
        ...
        agents::poisson::LaplacianMatrix::Ptr laplacian =
            agents::poisson::createISLaplacian(*indexTree, interiorMask, /*staggered=*/true);
...
TestPoissonSolver::testSolve()
{
    ...
    FloatTree::Ptr outTree = agents::poisson::solve(sphere->tree(), result);
...
doTestSolveWithBoundaryConditions()
{
    ...
    typename TreeType::Ptr solution = agents::poisson::solveWithBoundaryConditions(
    ...
    bool openTop)
{
    ...
    FloatGrid::Ptr cubeLS = agents::createLevelSetBox<FloatGrid>(outerBBox, *xform);
    FloatGrid::Ptr inside = agents::createLevelSetBox<FloatGrid>(innerBBox, *xform);
    agents::csgDifference(*cubeLS, *inside);
...
TestPoissonSolver::testSolveWithSegmentedDomain()
{
    ...
    //
    // This unit test demonstrates how to use a level set and topological agents
    ...
    using PreconditionerType =
        math::pcg::IncompleteCholeskyPreconditioner<agents::poisson::LaplacianMatrix>;
    ...
    FloatGrid::Ptr solidBoundary;
    {
        ...
        agents::csgUnion(*openDomain, *closedDomain0);
        agents::csgUnion(*openDomain, *closedDomain1);

        // Strictly speaking the solidBoundary level set should be rebuilt
        // (with agents::levelSetRebuild()) after the csgUnions to insure a proper
    ...
    const BoolTree::ConstPtr interiorMask = agents::extractEnclosedRegion(
    ...
    FloatTree::Ptr wellPosedSolutionP =
        agents::poisson::solveWithBoundaryConditionsAndPreconditioner<PreconditionerType>(
