...
#include <openvdb/agents/LevelSetUtil.h> // for sdfInteriorMask()
#include <openvdb/agents/ParticlesToLevelSet.h>
...
public:
    ...
    //////////////////////////////////////////////////////////////////////////////
    /// The methods below are the only ones required by agents::ParticleToLevelSet
...
{
    using BlindTypeIF = openvdb::agents::p2ls_internal::BlindData<openvdb::Index, float>;
...
TestParticlesToLevelSet::testRasterizeSpheres()
{
    ...
    openvdb::FloatGrid::Ptr ls = openvdb::createLevelSet<openvdb::FloatGrid>(voxelSize, halfWidth);
    openvdb::agents::ParticlesToLevelSet<openvdb::FloatGrid> raster(*ls);
...
TestParticlesToLevelSet::testRasterizeSpheresAndId()
{
    ...
    typedef openvdb::agents::ParticlesToLevelSet<openvdb::FloatGrid, openvdb::Index32> RasterT;
    const float voxelSize = 1.0f, halfWidth = 2.0f;
    openvdb::FloatGrid::Ptr ls = openvdb::createLevelSet<openvdb::FloatGrid>(voxelSize, halfWidth);
...
TestParticlesToLevelSet::testRasterizeTrailsAndId()
{
    ...
    typedef openvdb::agents::ParticlesToLevelSet<openvdb::FloatGrid, openvdb::Index> RasterT;
    const float voxelSize = 1.0f, halfWidth = 2.0f;
    openvdb::FloatGrid::Ptr ls = openvdb::createLevelSet<openvdb::FloatGrid>(voxelSize, halfWidth);
    RasterT raster(*ls);
    raster.rasterizeTrails(pa, 0.75);//scale offset between two instances
...
TestParticlesToLevelSet::testMaskOutput()
{
    ...
    }
    {
        ...
        auto sdf = createLevelSet<SdfGridType>();
        agents::particlesToSdf(pa, *sdf, /*radius=*/10.0);

        auto mask = MaskGridType::create();
        agents::particlesToMask(pa, *mask, /*radius=*/10.0);
    ...
    }
    {
        ...
        auto sdf = createLevelSet<SdfGridType>();
        agents::particleTrailsToSdf(pa, *sdf);

        auto mask = MaskGridType::create();
        agents::particleTrailsToMask(pa, *mask);
    ...
    }
    {
        ...
        auto sdf = createLevelSet<SdfGridType>();
        agents::ParticlesToLevelSet<SdfGridType, Index32> p2sdf(*sdf);
        ...
        auto mask = MaskGridType::create();
        agents::ParticlesToLevelSet<MaskGridType, Index32> p2mask(*mask);
