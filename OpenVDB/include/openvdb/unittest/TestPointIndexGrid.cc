...
TestPointIndexGrid::testPointIndexGrid()
{
    ...
    typedef openvdb::agents::PointIndexGrid PointIndexGrid;

    PointIndexGrid::Ptr pointGridPtr =
        openvdb::agents::createPointIndexGrid<PointIndexGrid>(pointList, *transform);
    ...
    typedef openvdb::agents::PointIndexIterator<> PointIndexIterator;
    ...
    CPPUNIT_ASSERT(openvdb::agents::isValidPartition(pointList, *pointGridPtr));
    ...
    PointIndexGrid::Ptr pointGrid2Ptr =
        openvdb::agents::getValidPointIndexGrid<PointIndexGrid>(pointList, pointGridPtr);

    CPPUNIT_ASSERT(openvdb::agents::isValidPartition(pointList, *pointGrid2Ptr));
}
...
TestPointIndexGrid::testPointIndexFilter()
{
    ...
    openvdb::agents::PointIndexFilter<PointList>
...
TestPointIndexGrid::testWorldSpaceSearchAndUpdate()
{
    ...
    openvdb::agents::PointIndexGrid::Ptr pointIndexGrid
        = openvdb::agents::createPointIndexGrid<openvdb::agents::PointIndexGrid, PointList>(pointArray, *transform);

    // Search for points within the box.
    openvdb::agents::PointIndexGrid::ConstAccessor acc = pointIndexGrid->getConstAccessor();

    openvdb::agents::PointIndexIterator<openvdb::agents::PointIndexTree> pointIndexIter;
