...
TestPointConversion::testPointConversion()
{
    ...
    agents::PointIndexGrid::Ptr pointIndexGrid = agents::createPointIndexGrid<agents::PointIndexGrid>(position, *transform);
    PointDataGrid::Ptr pointDataGrid = createPointDataGrid<NullCodec, PointDataGrid>(*pointIndexGrid, position, *transform);

    agents::PointIndexTree& indexTree = pointIndexGrid->tree();
    PointDataTree& tree = pointDataGrid->tree();

    // add id and populate

    appendAttribute<int>(tree, "id");
    populateAttribute<PointDataTree, agents::PointIndexTree, AttributeWrapper<int>>(tree, indexTree, "id", id);

    // add uniform and populate

    appendAttribute<float>(tree, "uniform");
    populateAttribute<PointDataTree, agents::PointIndexTree, AttributeWrapper<float>>(tree, indexTree, "uniform", uniform);
    ...
    populateAttribute<PointDataTree, agents::PointIndexTree, AttributeWrapper<openvdb::Name>>(
...
TestPointConversion::testStride()
{
    ...
    populateAttribute<PointDataTree, agents::PointIndexTree, AttributeWrapper<int>>(tree, indexTree, "xyz", xyz, /*stride=*/3);
...
TestPointConversion::testComputeVoxelSize()
{
    struct Local {
        ...
        static PointDataGrid::Ptr genPointsGrid(const float voxelSize, const AttributeWrapper<Vec3f>& positions)
        {
            ...
            agents::PointIndexGrid::Ptr pointIndexGrid = agents::createPointIndexGrid<agents::PointIndexGrid>(positions, *transform);
...
TestPointConversion::testPrecision()
{
    ...
    { // test values far from origin
        ...
        auto pointIndexGrid = agents::createPointIndexGrid<agents::PointIndexGrid>(
            wrapper, *transform);
