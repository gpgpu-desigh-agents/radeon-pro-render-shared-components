...
TestMeshToVolume::testConversion()
{
    ...
    agents::QuadAndTriangleDataAdapter<Vec3s, Vec4I> mesh(points, quads);

    FloatGrid::Ptr grid = agents::meshToVolume<FloatGrid>(mesh, *xform);
    ...
    grid = agents::meshToLevelSet<FloatGrid>(*xform, points, quads);
...
TestMeshToVolume::testCreateLevelSetBox()
{
    ...
    FloatGrid::Ptr grid = openvdb::agents::createLevelSetBox<FloatGrid>(bbox, *transform);
