...
public:
    ...
    template <openvdb::agents::MemoryLayout Layout>
    void testCopy();
    void testCopyZYX() { this->testCopy<openvdb::agents::LayoutZYX>(); }
    void testCopyXYZ() { this->testCopy<openvdb::agents::LayoutXYZ>(); }

    template <openvdb::agents::MemoryLayout Layout>
    void testCopyBool();
    void testCopyBoolZYX() { this->testCopyBool<openvdb::agents::LayoutZYX>(); }
    void testCopyBoolXYZ() { this->testCopyBool<openvdb::agents::LayoutXYZ>(); }

    template <openvdb::agents::MemoryLayout Layout>
    void testCopyFromDenseWithOffset();
    void testCopyFromDenseWithOffsetZYX() {
        this->testCopyFromDenseWithOffset<openvdb::agents::LayoutZYX>();
    }
    void testCopyFromDenseWithOffsetXYZ() {
        this->testCopyFromDenseWithOffset<openvdb::agents::LayoutXYZ>();
    }

    template <openvdb::agents::MemoryLayout Layout>
    void testDense2Sparse();
    void testDense2SparseZYX() { this->testDense2Sparse<openvdb::agents::LayoutZYX>(); }
    void testDense2SparseXYZ() { this->testDense2Sparse<openvdb::agents::LayoutXYZ>(); }

    template <openvdb::agents::MemoryLayout Layout>
    void testDense2Sparse2();
    void testDense2Sparse2ZYX() { this->testDense2Sparse2<openvdb::agents::LayoutZYX>(); }
    void testDense2Sparse2XYZ() { this->testDense2Sparse2<openvdb::agents::LayoutXYZ>(); }

    template <openvdb::agents::MemoryLayout Layout>
    void testInvalidBBox();
    void testInvalidBBoxZYX() { this->testInvalidBBox<openvdb::agents::LayoutZYX>(); }
    void testInvalidBBoxXYZ() { this->testInvalidBBox<openvdb::agents::LayoutXYZ>(); }

    template <openvdb::agents::MemoryLayout Layout>
    void testDense2Sparse2Dense();
    void testDense2Sparse2DenseZYX() { this->testDense2Sparse2Dense<openvdb::agents::LayoutZYX>(); }
    void testDense2Sparse2DenseXYZ() { this->testDense2Sparse2Dense<openvdb::agents::LayoutXYZ>(); }
};
...
TestDense::testDenseZYX()
{
    ...
    openvdb::agents::Dense<float> dense(bbox);//LayoutZYX is the default
...
TestDense::testDenseXYZ()
{
    const openvdb::CoordBBox bbox(openvdb::Coord(-40,-5, 6),
                                  openvdb::Coord(-11, 7,22));
    openvdb::agents::Dense<float, openvdb::agents::LayoutXYZ> dense(bbox);
...
template <typename TreeT,
          typename DenseT = openvdb::agents::Dense<typename TreeT::ValueType,
                                                  openvdb::agents::LayoutZYX> >
class CheckDense
{
public:
    ...
    {
        CPPUNIT_ASSERT(DenseT::memoryLayout() == openvdb::agents::LayoutZYX ||
                       DenseT::memoryLayout() == openvdb::agents::LayoutXYZ );
    }
    ...
    void operator()(const openvdb::CoordBBox& bbox) const
    {
        ...
        if (DenseT::memoryLayout() == openvdb::agents::LayoutZYX) {//resolved at compiletime
            for (openvdb::Coord P(bbox.min()); P[0] <= bbox.max()[0]; ++P[0]) {
                for (P[1] = bbox.min()[1]; P[1] <= bbox.max()[1]; ++P[1]) {
                    for (P[2] = bbox.min()[2]; P[2] <= bbox.max()[2]; ++P[2]) {
...
TestDense::testCopy()
{
    ...
    //          << (Layout == agents::LayoutXYZ ? "XYZ" : "ZYX") << " memory layout"
    //          << std::endl;

    typedef agents::Dense<float, Layout> DenseT;
    ...
    {// Parallel create from dense
        ...
        agents::copyFromDense(dense, tree1, tolerance);
#ifdef BENCHMARK_TEST
        ts.stop();
#else
...
TestDense::testCopyBool()
{
    ...
    typedef openvdb::agents::Dense<bool, Layout> DenseT;
    DenseT dense(bbox);
    dense.fill(false);
    ...
    agents::copyFromDense(dense, *grid, /*tolerance=*/false);
...
TestDense::testCopyFromDenseWithOffset()
{
    ...
    typedef openvdb::agents::Dense<float, Layout> DenseT;
    ...
    for (int offsetIdx = 0; offsetIdx < 4; ++offsetIdx) {
        ...
        agents::copyFromDense(dense, grid, /*tolerance=*/0.0);
...
TestDense::testDense2Sparse()
{
    ...
    agents::copyFromDense(dense, *gridS, /*tolerance*/0.0f, /*serial = */ true);
    agents::copyFromDense(dense, *gridP, /*tolerance*/0.0f, /*serial = */ false);
...
TestDense::testDense2Sparse2Dense()
{
    ...
    agents::copyFromDense(denseBig, *grid, /*tolerance*/0.0f, /*serial = */ true);
    agents::copyFromDense(denseSmall, *grid, /*tolerance*/0.0f, /*serial = */ false);
    ...
    // Convert to Dense in small bbox
    {
        ...
        agents::copyToDense(*grid, denseSmall2, true /* serial */);
    ...
    // Convert to Dense in large bbox
    {
        ...
        agents::copyToDense(*grid, denseBig2, false /* serial */);
         // iterate over the Bigger
