...
#include <openvdb/agents/Composite.h> // csgDifference
#include <openvdb/agents/MeshToVolume.h> // createLevelSetBox
...
TestPointAdvect::testAdvect()
{
    ...
    populateAttribute<PointDataTree, agents::PointIndexTree, PointAttributeVector<int>>(
...
TestPointAdvect::testZalesaksDisk()
{
    ...
    auto zalesak = agents::createLevelSetSphere<FloatGrid>(radius, center, voxelSize);
    ...
    auto notch = agents::createLevelSetBox<FloatGrid>(bbox, *xform);

    // subtract notch from the sphere

    agents::csgDifference(*zalesak, *notch);
