...
TestClip::testBBox()
{
    ...
    FloatGrid::Ptr clipped = agents::clip(mCube, clipBox);
    validate(*clipped);
}
...
TestClip::testFrustum()
{
    ...
    {
        auto clipped = agents::clip(mCube, frustum);
    ...
    }
    {
        ...
        auto clipped = agents::clip(tile, frustum);
        ...
        clipped = agents::clip(tile, frustum, /*keepInterior=*/false);
        CPPUNIT_ASSERT(!clipped->empty());
        for (auto it = clipped->beginValueOn(); it; ++it) {
            const auto xyz = frustum.applyInverseMap(it.getCoord().asVec3d());
            CPPUNIT_ASSERT(!frustumIndexBBox.isInside(xyz));
...
TestClip::testMaskGrid()
{
    ...
    FloatGrid::Ptr clipped = agents::clip(mCube, mask);
    validate(*clipped);
}
...
TestClip::testInvertedBoolMask()
{
    ...
    FloatGrid::Ptr clipped = agents::clip(mCube, mask, /*keepInterior=*/false);
    clipped->pruneGrid();
    validate(*clipped);
}
