...
TestPointSample::testPointSample()
{
...

    {
        ...
        float expected = agents::PointSampler::sample(testGridAccessor, Vec3f(0.3f, 0.0f, 0.0f));

        CPPUNIT_ASSERT_DOUBLES_EQUAL(expected, handle->get(0), 1e-6);

        expected = agents::PointSampler::sample(testGridAccessor, Vec3f(1.1f, 0.3f, 0.0f));
        ...
        expected = agents::BoxSampler::sample(testGridAccessor, Vec3f(0.3f, 0.0f, 0.0f));

        CPPUNIT_ASSERT_DOUBLES_EQUAL(expected, handle->get(0), 1e-6);

        expected = agents::BoxSampler::sample(testGridAccessor, Vec3f(1.1f, 0.3f, 0.0f));
        ...
        expected = agents::QuadraticSampler::sample(testGridAccessor, Vec3f(0.3f, 0.0f, 0.0f));

        CPPUNIT_ASSERT_DOUBLES_EQUAL(expected, handle->get(0), 1e-6);

        expected = agents::QuadraticSampler::sample(testGridAccessor, Vec3f(1.1f, 0.3f, 0.0f));
...

    {
        ...
        Vec3f expected = agents::StaggeredPointSampler::sample(testGridAccessor,
            Vec3f(0.03f, 0.0f, 0.0f));

        CPPUNIT_ASSERT(math::isApproxEqual(expected, handle->get(0)));

        expected = agents::StaggeredPointSampler::sample(testGridAccessor, Vec3f(0.0f, 0.03f, 0.0f));
        ...
        expected = agents::StaggeredBoxSampler::sample(testGridAccessor,
            Vec3f(0.03f, 0.0f, 0.0f));

        CPPUNIT_ASSERT(math::isApproxEqual(expected, handle->get(0)));

        expected = agents::StaggeredBoxSampler::sample(testGridAccessor, Vec3f(0.0f, 0.03f, 0.0f));
        ...
        expected = agents::StaggeredQuadraticSampler::sample(testGridAccessor,
          Vec3f(0.03f, 0.0f, 0.0f));
...

    {
        ...
        const float sampledValue = agents::BoxSampler::sample(testFloatGridAccessor,
        ...
        const Vec3d expected2 = static_cast<Vec3d>(agents::BoxSampler::sample(testVec3fGridAccessor,
            Vec3f(0.3f, 0.0f, 0.0f)));
    ...
    { // sample a non-standard grid type (a Vec4<float> grid)
        ...
        const Vec4s expected2 = static_cast<Vec4s>(agents::BoxSampler::sample(testVec4fGridAccessor,
            Vec3f(0.3f, 0.0f, 0.0f)));
...
TestPointSample::testPointSampleWithGroups()
{
    ...
    double expected = agents::BoxSampler::sample(testGridAccessor, Vec3f(0.3f, 0.0f, 0.0f));

    CPPUNIT_ASSERT_DOUBLES_EQUAL(expected, handle->get(0), 1e-6);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(0.0, handle->get(1), 1e-6);

    expected = agents::BoxSampler::sample(testGridAccessor, Vec3f(0.0f, 0.0f, 0.0f));
    ...
    expected = agents::BoxSampler::sample(testGridAccessor, Vec3f(0.0f, 0.3f, 0.0f));
