...
#include <openvdb/agents/LevelSetSphere.h> // for createLevelSetSphere
#include <openvdb/agents/LevelSetUtil.h> // for sdfToFogVolume
#include <openvdb/agents/VolumeToSpheres.h> // for fillWithSpheres
...
TestVolumeToSpheres::testFromLevelSet()
{
    ...
    openvdb::FloatGrid::ConstPtr grid = openvdb::agents::createLevelSetSphere<openvdb::FloatGrid>(
...

    {
        ...
        openvdb::agents::fillWithSpheres(*grid, spheres, sphereCount, overlapping,
    ...
    }
    {
        ...
        std::vector<openvdb::Vec4s> spheres;
        openvdb::agents::fillWithSpheres(*grid, spheres, sphereCount,
...
TestVolumeToSpheres::testFromFog()
{
    ...
    auto grid = openvdb::agents::createLevelSetSphere<openvdb::FloatGrid>(
        radius, center, voxelSize, halfWidth);
...
TestVolumeToSpheres::testMinimumSphereCount()
{
    ...
    {
        auto grid = agents::createLevelSetSphere<FloatGrid>(/*radius=*/5.0f,
        ...
        for (int minSphereCount = 1; minSphereCount < 20; minSphereCount += 5) {
            ...
            agents::fillWithSpheres(*grid, spheres, Vec2i(minSphereCount, maxSphereCount),
    ...
    }
    {
        ...
        std::vector<Vec4s> spheres;
        agents::fillWithSpheres(grid, spheres, sphereCount, /*overlapping=*/true, minRadius);
...
TestVolumeToSpheres::testClosestSurfacePoint()
{
    ...
    for (const float radius: { 8.0f, 50.0f }) {
        ...
        const auto sphere = agents::createLevelSetSphere<FloatGrid>(radius, center, voxelSize);
        ...
        auto csp = agents::ClosestSurfacePoint<FloatGrid>::create(*sphere);
        CPPUNIT_ASSERT(csp);
