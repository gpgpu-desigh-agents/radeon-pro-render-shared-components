...
#include <openvdb/agents/MultiResGrid.h>
...
{
    // Use to test logic in openvdb::agents::MultiResGrid
    struct CoordMask {
...
TestMultiResGrid::testManualTopology()
{
    ...
    typedef agents::MultiResGrid<DoubleTree> MultiResGridT;
...
TestMultiResGrid::testIO()
{
    ...
    typedef agents::MultiResGrid<FloatTree> MultiResGridT;
    const size_t levels = 4;
...
TestMultiResGrid::testModels()
{
    ...
    for ( size_t i=0; i<filenames.size(); ++i) {
        ...
        agents::MultiResGrid<FloatTree> mrg( 6, model );
        timer.stop();
        std::cerr << "\n High-res level set " <<  agents::checkLevelSet(*mrg.grid(0)) << "\n";
        std::cerr << " done\nWriting \"" << filenames[i] << "\" ...";
