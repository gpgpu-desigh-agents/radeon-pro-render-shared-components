...
TestStats::testGridExtrema()
{
    ...
    const int DIM = 109;
    {
        ...
        FloatGrid grid(background);
        {
            ...
            math::Extrema ex = agents::extrema(grid.cbeginValueOn());
            ...
            ex = agents::extrema(grid.cbeginValueOff());
        ...
        for (int threaded = 0; threaded <= 1; ++threaded) {
            math::Extrema ex = agents::extrema(grid.cbeginValueOn(), threaded);
        ...
        for (int threaded = 0; threaded <= 1; ++threaded) {
            ...
            math::Extrema ex =
                agents::extrema(grid.cbeginValueOn(), &Local::addIfPositive, threaded);
        ...
        for (int threaded = 0; threaded <= 1; ++threaded) {
            ...
            math::Extrema ex = agents::extrema(grid.cbeginValueOn(), GradOp(grid), threaded);
            ...
            // ...then using agents::opStatistics().
            typedef math::ISOpMagnitude<math::ISGradient<math::FD_1ST> > MathOp;
            ex = agents::opExtrema(grid.cbeginValueOn(), MathOp(), threaded);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(double(0.0), ex.min(), /*tolerance=*/0.0);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(
...
TestStats::testGridStats()
{
    ...
    const int DIM = 109;
    {
        ...
        FloatGrid grid(background);
        {
            // Compute active value statistics for a grid with a single active voxel.
            grid.tree().setValue(Coord(0), /*value=*/42.0);
            math::Stats stats = agents::statistics(grid.cbeginValueOn());
            ...
            // Compute inactive value statistics for a grid with only background voxels.
            grid.tree().setValueOff(Coord(0), background);
            stats = agents::statistics(grid.cbeginValueOff());
        ...
        for (int threaded = 0; threaded <= 1; ++threaded) {
            math::Stats stats = agents::statistics(grid.cbeginValueOn(), threaded);
        ...
        for (int threaded = 0; threaded <= 1; ++threaded) {
            ...
            math::Stats stats =
                agents::statistics(grid.cbeginValueOn(), &Local::addIfPositive, threaded);
        ...
        for (int threaded = 0; threaded <= 1; ++threaded) {
            // First, using a custom ValueOp...
            math::Stats stats = agents::statistics(grid.cbeginValueOn(), GradOp(grid), threaded);
            ...
            // ...then using agents::opStatistics().
            typedef math::ISOpMagnitude<math::ISGradient<math::FD_1ST> > MathOp;
            stats = agents::opStatistics(grid.cbeginValueOn(), MathOp(), threaded);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(double(0.0), stats.min(), /*tolerance=*/0.0);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(
...
doTestGridOperatorStats(const GridT& grid, const OpT& op)
{
    openvdb::math::Stats serialStats =
        openvdb::agents::opStatistics(grid.cbeginValueOn(), op, /*threaded=*/false);

    openvdb::math::Stats parallelStats =
        openvdb::agents::opStatistics(grid.cbeginValueOn(), op, /*threaded=*/true);
...
TestStats::testGridHistogram()
{
    ...
    const int DIM = 109;
    {
        ...
        FloatGrid grid(background);
        {
            ...
            math::Histogram hist = agents::histogram(grid.cbeginValueOn(),
                /*min=*/0.0, /*max=*/100.0);
