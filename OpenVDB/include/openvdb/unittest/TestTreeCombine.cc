...
#include <openvdb/agents/Composite.h>
...
template<typename TreeT> void compMax(TreeT& a, TreeT& b) { openvdb::agents::compMax(a, b); }
template<typename TreeT> void compMin(TreeT& a, TreeT& b) { openvdb::agents::compMin(a, b); }
template<typename TreeT> void compSum(TreeT& a, TreeT& b) { openvdb::agents::compSum(a, b); }
template<typename TreeT> void compMul(TreeT& a, TreeT& b) { openvdb::agents::compMul(a, b); }\
template<typename TreeT> void compDiv(TreeT& a, TreeT& b) { openvdb::agents::compDiv(a, b); }\
...
TestTreeCombine::testBoolTree()
{
    ...
    CPPUNIT_ASSERT_THROW(openvdb::agents::csgUnion(aGrid->tree(), bGrid->tree()),
        openvdb::ValueError);
    CPPUNIT_ASSERT_THROW(openvdb::agents::csgIntersection(aGrid->tree(), bGrid->tree()),
        openvdb::ValueError);
    CPPUNIT_ASSERT_THROW(openvdb::agents::csgDifference(aGrid->tree(), bGrid->tree()),
        openvdb::ValueError);

    openvdb::agents::compSum(aGrid->tree(), bGrid->tree());

    bGrid = sphere->copy();
    openvdb::agents::compMax(aGrid->tree(), bGrid->tree());
...
TestTreeCombine::testCompRepl()
{
...

    {
        ...
        openvdb::agents::compReplace(aTree, bTree);
    ...
    // As above, but combining the A grid into the B grid
    {
        ...
        openvdb::agents::compReplace(bTree, aTree);
...
TestTreeCombine::testCsg()
{
    ...
    struct Local {
        ...
        static void visitorUnion(TreeT& a, TreeT& b) { openvdb::agents::csgUnion(a, b); }
        static void visitorIntersect(TreeT& a, TreeT& b) { openvdb::agents::csgIntersection(a, b); }
        static void visitorDiff(TreeT& a, TreeT& b) { openvdb::agents::csgDifference(a, b); }
    };
    ...
    const std::string testDir("/work/rd/fx_agents/vdb_unittest/TestGridCombine::testCsg/");
...
TestTreeCombine::testCsgCopy()
{
    ...
    openvdb::FloatGrid::Ptr gridA =
        openvdb::agents::createLevelSetSphere<openvdb::FloatGrid>(radius, center, voxelSize);
    ...
    openvdb::FloatGrid::Ptr unionGrid = openvdb::agents::csgUnionCopy(*gridA, *gridB);
    openvdb::FloatGrid::Ptr intersectionGrid = openvdb::agents::csgIntersectionCopy(*gridA, *gridB);
    openvdb::FloatGrid::Ptr differenceGrid = openvdb::agents::csgDifferenceCopy(*gridA, *gridB);
...
TestTreeCombine::testCompActiveLeafVoxels()
{
    {//replace float tree (default argument)
        ...
        openvdb::agents::compActiveLeafVoxels(srcTree, dstTree);
    ...
    {//replace float tree (lambda expression)
        ...
        openvdb::agents::compActiveLeafVoxels(srcTree, dstTree, [](float &d, float s){d=s;});
    ...
    {//replace bool tree
        ...
        openvdb::agents::compActiveLeafVoxels(srcTree, dstTree, [](Word &d, Word s){d=s;});
