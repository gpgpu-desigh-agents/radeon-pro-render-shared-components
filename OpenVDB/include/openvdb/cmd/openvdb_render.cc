...
#include <openvdb/agents/RayIntersector.h>
#include <openvdb/agents/RayTracer.h>
...
usage [[noreturn]] (int exitStatus = EXIT_FAILURE)
{
    ...
    const double fov = openvdb::agents::PerspectiveCamera::focalLengthToFieldOfView(
...
void
saveEXR(const std::string& fname, const openvdb::agents::Film& film, const RenderOpts& opts)
{
    using RGBA = openvdb::agents::Film::RGBA;
...
render(const GridType& grid, const std::string& imgFilename, const RenderOpts& opts)
{
    ...
    agents::Film film(opts.width, opts.height);

    std::unique_ptr<agents::BaseCamera> camera;
    if (boost::starts_with(opts.camera, "persp")) {
        camera.reset(new agents::PerspectiveCamera(film, opts.rotate, opts.translate,
            opts.focal, opts.aperture, opts.znear, opts.zfar));
    } else if (boost::starts_with(opts.camera, "ortho")) {
        camera.reset(new agents::OrthographicCamera(film, opts.rotate, opts.translate,
    ...
    std::unique_ptr<agents::BaseShader> shader;
    if (opts.shader == "matte") {
        if (opts.colorgrid) {
            shader.reset(new agents::MatteShader<openvdb::Vec3SGrid>(*opts.colorgrid));
        } else {
            shader.reset(new agents::MatteShader<>());
        }
    } else if (opts.shader == "normal") {
        if (opts.colorgrid) {
            shader.reset(new agents::NormalShader<Vec3SGrid>(*opts.colorgrid));
        } else {
            shader.reset(new agents::NormalShader<>());
        }
        ...
        if (opts.colorgrid) {
            shader.reset(new agents::PositionShader<Vec3SGrid>(bboxWorld, *opts.colorgrid));
        } else {
            shader.reset(new agents::PositionShader<>(bboxWorld));
        }
    } else /* if (opts.shader == "diffuse") */ { // default
        if (opts.colorgrid) {
            shader.reset(new agents::DiffuseShader<Vec3SGrid>(*opts.colorgrid));
        } else {
            shader.reset(new agents::DiffuseShader<>());
        }
    }
    ...
    if (isLevelSet) {
        agents::LevelSetRayIntersector<GridType> intersector(
            grid, static_cast<typename GridType::ValueType>(opts.isovalue));
        agents::rayTrace(grid, intersector, *shader, *camera, opts.samples,
            /*seed=*/0, (opts.threads != 1));
    } else {
        using IntersectorType = agents::VolumeRayIntersector<GridType>;
        IntersectorType intersector(grid);

        agents::VolumeRender<IntersectorType> renderer(intersector, *camera);
...
main(int argc, char *argv[])
{
    ...
    if (hasFov) {
        ...
        opts.focal = float(
            openvdb::agents::PerspectiveCamera::fieldOfViewToFocalLength(fov, opts.aperture));
    }
