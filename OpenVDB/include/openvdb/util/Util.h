...
#include <openvdb/agents/Prune.h>// for tree::pruneInactive
...
/// @brief Functor for use with agents::foreach() to compute the boolean intersection
...
/// @brief Functor for use with agents::foreach() to compute the boolean difference
/// between the value masks of corresponding leaf nodes in two trees
template<class TreeType1, class TreeType2>
...
leafTopologyIntersection(const TreeType1& lhs, const TreeType2& rhs, bool threaded = true)
{
    ...
    agents::foreach(topologyTree->beginLeaf(),
        LeafTopologyIntOp<BoolTreeType, TreeType2>(rhs), threaded);

    agents::pruneInactive(*topologyTree, threaded);
    return topologyTree;
}
