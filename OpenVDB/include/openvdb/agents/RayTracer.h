...
template<typename GridT, typename IntersectorT = agents::LevelSetRayIntersector<GridT> >
...
template <typename IntersectorT, typename SamplerT = agents::BoxSampler>
class VolumeRender
{
public:
    ...
    using SamplerType = agents::GridSampler<AccessorType, SamplerT>;
...
template<typename GridT = Film::RGBA,
         typename SamplerType = agents::PointSampler>
...
inline void rayTrace(const GridT& grid,
                     ...
                     bool threaded)
{
    LevelSetRayTracer<GridT, agents::LevelSetRayIntersector<GridT> >
