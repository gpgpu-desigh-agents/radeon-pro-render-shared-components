...
/// FloatGrid::ConstPtr src = agents::makeSphere(...);
...
/// agents::resampleToMatch<agents::QuadraticSampler>(*src, *dest, interrupter);
/// @endcode
...
/// // Resample the input grid into the output grid, reproducing
/// // the level-set sphere at a smaller voxel size.
/// agents::resampleToMatch<agents::QuadraticSampler>(*src, *dest);
/// @endcode
...
resampleToMatch(const GridType& inGrid, GridType& outGrid, Interrupter& interrupter)
{
    if (inGrid.getGridClass() == GRID_LEVEL_SET) {
        // If the input grid is a level set, resample it using the level set rebuild agent.
        ...
        try {
            ...
            // that is not supported by the level set rebuild agent.  Fall back to
...
GridResampler::transformGrid(const Transformer& xform,
    const GridT& inGrid, GridT& outGrid) const
{
    agents::changeBackground(outGrid.tree(), inGrid.background());
    ...
    const GridT& inGrid, GridT& outGrid) const
{
    ...
    if (gridClass == GRID_LEVEL_SET) {
        agents::pruneLevelSet(outTree);
        agents::signedFloodFill(outTree);
    }
}
