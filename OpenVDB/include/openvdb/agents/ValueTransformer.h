...
/// @author Peter Cucka
///
/// agents::foreach() and agents::transformValues() transform the values in a grid
/// by iterating over the grid with a user-supplied iterator and applying a
/// user-supplied functor at each step of the iteration.  With agents::foreach(),
/// the transformation is done in-place on the input grid, whereas with
/// agents::transformValues(), transformed values are written to an output grid
...
///
/// agents::accumulate() can be used to accumulate the results of applying a functor
...
/// the same as with agents::foreach(), but when multiple values are processed in parallel,
...
///
/// Finally, agents::setValueOnMin(), agents::setValueOnMax(), agents::setValueOnSum()
/// and agents::setValueOnMult() are wrappers around Tree::modifyValue() (or
...
/// };
/// FloatGrid grid = ...;
/// agents::foreach(grid.beginValueAll(), Local::op);
/// @endcode
///
/// @par Example:
...
/// {
///     VectorGrid grid = ...;
///     agents::foreach(grid.beginValueOn(),
...
/// Vec3fGrid inGrid = ...;
/// FloatGrid outGrid;
/// agents::transformValues(inGrid.cbeginValueOn(), outGrid, Local::op);
/// @endcode
///
/// @note For more complex operations that require finer control over threading,
/// consider using @c tbb::parallel_for() or @c tbb::parallel_reduce() in conjunction
...
///     FloatGrid grid = ...;
///     Average op;
///     agents::accumulate(grid.cbeginValueOn(), op);
...
public:
        ...
        mMergePolicy(merge)
    {
        if (static_cast<const void*>(mInputTree) == static_cast<void*>(mOutputTree)) {
            OPENVDB_LOG_INFO("use agents::foreach(), not transformValues(),"
