...
{
    OPENVDB_THROW(NotImplementedError, "agents::erode18 is not implemented yet!");
}
...
{
    OPENVDB_THROW(NotImplementedError, "agents::erode26 is not implemented yet!");
}
...
Morphology<TreeType>::doErosion(NearestNeighbors nn)
{
    ...
    agents::pruneLevelSet(mManager->tree());
}
...
class DilationOp
{
    ...
    const agents::NearestNeighbors mNN;//enum to specify the dilation scheme
...
public:
        ...
        : mIter(iterations), mNN(nn), mPool(nullptr), mLeafs(nullptr)
    {
        ...
        if (mode == PRESERVE_TILES) agents::prune(mask);//multithreaded
    ...
    void operator()(const tbb::blocked_range<LeafT**> &r) const
    {
        ...
        agents::dilateVoxels(manager, mIter, mNN);// serial dilation of active voxels
