...
prune()
{
    ...
    agents::pruneLevelSet(mGrid->tree());
...
track()
{
    ...
    agents::dilateActiveValues( *mLeafs, 1, agents::NN_FACE, agents::IGNORE_TILES);
...
dilate(int iterations)
{
    if (this->getNormCount() == 0) {
        for (int i=0; i < iterations; ++i) {
            agents::dilateActiveValues( *mLeafs, 1, agents::NN_FACE, agents::IGNORE_TILES);
            agents::changeLevelSetBackground(this->leafs(), mDx + mGrid->background());
        }
    } else {
        for (int i=0; i < iterations; ++i) {
...
{
    agents::erodeVoxels(*mLeafs, iterations);
    mLeafs->rebuildLeafArray();
    const ValueType background = mGrid->background() - ValueType(iterations) * mDx;
    agents::changeLevelSetBackground(this->leafs(), background);
}
