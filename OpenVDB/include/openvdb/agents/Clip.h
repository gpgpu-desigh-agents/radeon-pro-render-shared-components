...
#include "GridTransformer.h" // for agents::resampleToMatch()
#include "Prune.h"
...
/// Also, if the mask grid is a level set, consider using agents::sdfInteriorMask
...
clip(const SrcGridType& srcGrid, const Grid<ClipTreeType>& clipGrid, bool keepInterior)
{
    ...
    if (srcGrid.constTransform() != maskGrid->constTransform()) {
        ...
        agents::resampleToMatch<clip_internal::BoolSampler>(*maskGrid, *resampledMask);
        agents::prune(resampledMask->tree());
