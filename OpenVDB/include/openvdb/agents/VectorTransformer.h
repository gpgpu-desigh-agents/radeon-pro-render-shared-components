...
#include "ValueTransformer.h" // for agents::foreach()
#include <type_traits>
...
// Functors for use with agents::foreach() to transform vector voxel values
...
{
    OPENVDB_THROW(TypeError, "agents::transformVectors() requires a vector-valued grid");
}
