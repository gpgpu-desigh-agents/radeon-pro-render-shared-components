...
///
/// @brief   This agent generates a narrow-band signed distance field / level set
///          from the interface between active and inactive voxels in a vdb grid.
///
/// @par Example:
/// Combine with @c agents::PointsToVolume for fast point cloud to level set conversion.
...
struct DilateOp
{
    ...
    void operator()() const {
        dilateActiveValues( *tree, size, agents::NN_FACE, agents::IGNORE_TILES);
    }
    TreeT* tree;
    const int size;
};
    ...
    int smoothingSteps, InterrupterT* interrupt)
{
    ...
    dilateActiveValues( maskTree, closingSteps + dilation, agents::NN_FACE, agents::IGNORE_TILES );
    ...
    agents::pruneLevelSet( *lsTree,  /*threading=*/true);
