...
/// @file agents/PotentialFlow.h
///
/// @brief Agents for creating potential flow fields through solving Laplace's equation
///
    ...
    const Vec3T& backgroundVelocity)
{
    ...
    // use the agents::topologyToLevelset() method if you need to convert a mask into a level set
    ...
    const typename GradientT::Ptr gradient = agents::gradient(collider);
    ...
    agents::pruneInactive(*neumannTree);
    ...
    const typename Vec3GridT::ValueType backgroundVelocity)
{
    ...
    auto gradient = agents::gradient(potential);
