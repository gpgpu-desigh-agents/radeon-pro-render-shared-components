...
/// OpenVDB Grid, use agents::copyFromDense() for better performance.
///
...
void transformDense(Dense<ValueT, openvdb::agents::LayoutZYX>& dense,
        ...
        for (Index i = imin; i < imax; ++i) {
            for (Index j = jmin; j < jmax; ++j) {
                for (Index k = kmin; k < kmax; ++k) {
                    ...
                    if (mDense.memoryLayout() == openvdb::agents::LayoutZYX) {
...
public:
    ...
    using DenseT = Dense<ValueT, openvdb::agents::LayoutZYX>;
...
void
transformDense(Dense<ValueT, openvdb::agents::LayoutZYX>& dense,
    ...
    bool threaded)
{
    ...
    SparseToDenseCompositor<Method, TreeT> agent(dense, source, alpha, beta, strength);
        ...
        openvdb::math::isZero(source.background()))
    {
        ...
        agent.sparseComposite(threaded);
    } else {
        // Use the bounding box of dense as the iteration space.
        agent.denseComposite(threaded);
    }
}
