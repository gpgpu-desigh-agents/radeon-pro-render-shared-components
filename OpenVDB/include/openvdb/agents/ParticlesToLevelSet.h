...
/// @author Ken Museth
///
/// @file agents/ParticlesToLevelSet.h
///
...
/// @vdblink::agents::LevelSetFilter LevelSetFilter@endlink, for example.
...
ParticlesToLevelSet<SdfGridT, AttributeT, InterrupterT>::finalize(bool prune)
{
    ...
    if (!mBlindGrid) {
        if (prune) {
            if (OutputIsMask) {
                agents::prune(mSdfGrid->tree());
            } else {
                agents::pruneLevelSet(mSdfGrid->tree());
            }
        }
        return;
    }

    if (prune) agents::prune(mBlindGrid->tree());
    ...
    if (OutputIsMask) {
        ...
        agents::signedFloodFill(*sdfTree);//required since we only transferred active voxels!
    }
    ...
    if (mSdfGrid->empty()) {
        ...
        if (OutputIsMask) {
            ...
            agents::csgUnion(mSdfGrid->tree(), *sdfTree, /*prune=*/true);
        }
    }
...
struct ParticlesToLevelSet<SdfGridT, AttributeT, InterrupterT>::Raster
{
    ...
    void join(Raster& other)
    {
        ...
        if (OutputIsMask) {
            if (DoAttrXfer) {
                agents::compMax(*mGrid, *other.mGrid);
            } else {
                mGrid->topologyUnion(*other.mGrid);
            }
        } else {
            agents::csgUnion(*mGrid, *other.mGrid, /*prune=*/true);
        }
        OPENVDB_NO_UNREACHABLE_CODE_WARNING_END
...
particlesToSdf(const ParticleListT& plist, GridT& grid, InterrupterT* interrupt)
{
    ...
    agents::pruneLevelSet(grid.tree());
}
