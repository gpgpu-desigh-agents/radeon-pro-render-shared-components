  ...
  typename GridType::template ValueConverter<Int32>::Type * polygonIndexGrid)
{
    ...
    if (computeSignedDistanceField) {
        ...
        if (removeIntersectingVoxels) {
            ...
            agents::pruneInactive(distTree,  /*threading=*/true);
            agents::pruneInactive(indexTree, /*threading=*/true);
        }
    }
    ...
    if (computeSignedDistanceField) {
        distTree.root().setBackground(exteriorWidth, /*updateChildNodes=*/false);
        agents::signedFloodFillWithValues(distTree, exteriorWidth, -interiorWidth);
    } else {
        agents::changeBackground(distTree, exteriorWidth);
    }
