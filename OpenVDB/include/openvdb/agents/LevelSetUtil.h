    ...
    const typename TreeType::template ValueConverter<bool>::Type* fillMask)
{
    ...
    agents::traceExteriorBoundaries(*maskTree);
    ...
    agents::signedFloodFill(*maskTree);
...
struct FloodFillSign
{
    ...
    void operator()(const tbb::blocked_range<size_t>& range) const {
        ...
        for (size_t n = range.begin(), N = range.end(); n < N; ++n) {
            agents::signedFloodFillWithValues(*mSegments[n], exteriorValue, interiorValue);
        }
    }
    ...
    std::vector<typename GridOrTreeType::template ValueConverter<bool>::Type::Ptr>& masks)
{
    ...
    agents::pruneInactive(topologyMask);
