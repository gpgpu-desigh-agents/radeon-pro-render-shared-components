...
#define DWA_BOOST_VERSION (10 * BOOST_VERSION)
  ...
  #endif
  #include "openvdb/agents/MeshToVolume.h"
  #include "openvdb/agents/VolumeToMesh.h" // for agents::volumeToMesh()
#endif
...
#include "openvdb/agents/SignedFloodFill.h"
#include "pyutil.h"
...
{
    agents::changeBackground(grid.tree(), extractValueArg<GridType>(obj, "setBackground"));
}
...
{
    agents::prune(grid.tree(), extractValueArg<GridType>(tolerance, "prune"));
}
...
{
    if (valObj.is_none()) {
        agents::pruneInactive(grid.tree());
    } else {
        agents::pruneInactiveWithValue(
...
{
    agents::signedFloodFill(grid.tree());
}
...
protected:
    ...
    {
        validate();
        agents::Dense<ArrayValueType> valArray(mBBox, static_cast<ArrayValueType*>(mArray));
        agents::copyFromDense(valArray, *mGrid, mTolerance);
    }
    ...
    void toArray() const
    {
        validate();
        agents::Dense<ArrayValueType> valArray(mBBox, static_cast<ArrayValueType*>(mArray));
        agents::copyToDense(*mGrid, valArray);
    }
...
/// call agents::meshToLevelSet() to generate a level set grid.
template<typename GridType>
inline typename GridType::Ptr
    ...
    py::object xformObj, py::object halfWidthObj)
{
    ...
    return agents::meshToLevelSet<GridType>(*xform, points, triangles, quads, halfWidth);
}
...
volumeToQuadMesh(const GridType& grid, py::object isovalueObj)
{
    ...
    std::vector<Vec3s> points;
    std::vector<Vec4I> quads;
    agents::volumeToMesh(grid, points, quads, isovalue);
...
volumeToMesh(const GridType& grid, py::object isovalueObj, py::object adaptivityObj)
{
    ...
    std::vector<Vec3I> triangles;
    std::vector<Vec4I> quads;
    agents::volumeToMesh(grid, points, triangles, quads, isovalue, adaptivity);
...
{
    return agents::createLevelSetSphere<GridType>(radius, center, voxelSize, halfWidth);
}
